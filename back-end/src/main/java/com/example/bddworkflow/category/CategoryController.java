...
public class CategoryController {
    ...
    public ResponseEntity<CategoryResponse> updateCategory(
            ...
            @io.swagger.v3.oas.annotations.parameters.RequestBody(
                    description = "카테고리 수정 요청 (부분 수정). 누락 필드는 변경되지 않고, color/description은 명시적 null로 비운다.",
                    content = @Content(schema = @Schema(implementation = UpdateCategoryRequest.class)))
            @RequestBody ObjectNode body) {
        return ResponseEntity.ok(categoryService.updateCategory(userId, categoryId, body));
    }
