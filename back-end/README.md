...
- 커버리지 확인: `tools/trace-requirements.mjs`가 요건 카드, JavaParser source index, 테스트 결과를 병합
...
테스트 실행 후 하네스 품질 게이트까지 확인한다.

```bash
./gradlew validateHarness
```

리포트는 `build/harness/trace-report.md`와 `build/harness/trace-report.json`에 생성된다. 테스트를 아직 실행하지 않았다면 결과는 `NOT_RUN`으로 표시된다.

초기 실행에서 `node tools/trace-requirements.mjs`만 먼저 실행하면 JavaParser source index나 테스트 결과 파일이 없어 실패하거나 `RED / NOT_RUN`으로 보일 수 있다. `./gradlew traceRequirements` 또는 `./gradlew validateHarness`를 사용하면 필요한 source index가 먼저 생성된다.
