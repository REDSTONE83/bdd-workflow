...
const DEFAULT_OPENAPI_INDEX = resolve(
  workspaceRoot,
  "build/harness/indexes/openapi.index.json",
)
const DEFAULT_OUT_DIR = resolve(frontEndRoot, "src/api/generated")
...
function assertGeneratedBoundary(outDir) {
  ...
  if (relative(resolve(frontEndRoot, "src/api/generated"), outDir) !== "") {
    throw new Error("Output directory must be front-end/src/api/generated")
  }
}
...
async function materialize({ openapiIndex, outDir, check }) {
  assertGeneratedBoundary(outDir)
