...
function parseCliArgs(argv) {
  ...
  cfg.outFile ||= path.join(cfg.repoRoot, "build", "app", "indexes", "front-end.source-index.json")
  return cfg
}
  ...
  function visit(node) {
    if (ts.isCallExpression(node) && isDirectFetchExpression(node.expression)) {
      issues.push({
        ...
        message: "Application source must call HTTP APIs through app/front-end/src/api/**, not direct fetch.",
        location: {
          file: fileRel,
          line: lineOf(sourceFile, node),
        ...
        evidence: {
          allowedBoundary: "app/front-end/src/api/**",
