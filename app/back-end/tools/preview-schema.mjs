...
const outputDir = process.env.HARNESS_OUTPUT_ROOT
    ? path.resolve(process.env.HARNESS_OUTPUT_ROOT)
    : path.join(repoRoot, 'build', 'app');
const sourceIndexPath = path.join(outputDir, 'indexes', 'backend.source-index.json');

if (!fs.existsSync(sourceIndexPath)) {
    console.error(
        `Missing source index: ${sourceIndexPath}\n` +
        'Run npm run app:source-index first.'
    );
    process.exit(1);
}
