...
공통 기본 UI 컴포넌트는 `src/components/ui/`에 둔다.
...
- 화면 구역을 카드처럼 띄우지 않는다. 카드는 반복 항목, 대화상자 내부, 명확히 프레임이 필요한 도구에만 쓴다.
- card 안에 card를 중첩하지 않는다.

### 보호 앱 셸

인증된 업무 화면이 둘 이상으로 늘어나면 상단 헤더와 1차 내비를 공유하는 보호 앱 셸을 둔다. 이 앱 셸은 프로젝트 공통 레이아웃이지 shadcn/ui 기본 컴포넌트가 아니므로 `src/components/`에 두고, `src/components/ui/`로 올리지 않는다.
...
- 같은 레이아웃이 두 개 이상의 실제 보호 화면에서 안정적으로 쓰이기 전에는 더 일반적인 레이아웃 기본 컴포넌트로 추상화하지 않는다.

참조 구현:

- 앱 셸: `app/front-end/src/components/ProtectedLayout.tsx`
- 기존 인증 헤더: `app/front-end/src/components/ProtectedHeader.tsx`
- 활성 route Storybook: `app/front-end/src/components/ProtectedLayout.stories.tsx`
...
- 테이블, 대화상자, 드로어가 기준 화면 안에서 조작 가능한가
...
- 큰 제목 글꼴은 실제 첫 화면 강조 영역에만 쓰고, 대시보드/카드/사이드바 내부 제목은 더 작은 체계를 쓴다.
...
- 양식 오류는 입력과 연결한다.
- 되돌릴 수 없는 동작은 취소 가능한 확인 절차를 둔다.
- 일시 알림은 상태 통지에 쓰고, 사용자가 수정해야 하는 오류는 화면 안에 남긴다.

### 양식 전체 서버 오류 경고 안내

특정 입력에 귀속되지 않는 서버 오류(중복 자원 거절, 인증 실패, 일시 실패 등)는 양식 전체 경고 안내로 표시한다. 화면 간 시각 위계와 사용자 행동 안내를 일정하게 유지하기 위한 표준이다.

- 컨테이너: `<Alert variant="destructive">` (회복 가능한 일시 오류여도 사용자가 인지하고 행동을 정해야 하므로 위험 강조 variant를 사용한다).
- 아이콘: `lucide-react` 의 `AlertCircle` 을 기본으로 사용한다. 아이콘은 `aria-hidden="true"` 로 두고 의미는 `AlertTitle` 이 전달한다.
- 제목: `<AlertTitle>` 한 줄로 상태를 요약한다. 표준 용어 사전(`harness/docs/terminology/`)에 등록된 표현과 같은 단어를 쓴다 (예: `account.duplicateEmail` → "이미 등록된 이메일입니다").
...
- `<AlertDescription>` 한 줄만 두는 최소 구성은 양식 전체 오류에서 금지한다. 입력별 검증 안내(`fieldErrors`)는 경고 안내가 아니라 해당 입력 아래의 `<p>`로 표시한다.
- 대화상자 안의 양식 전체 오류도 같은 구조를 따른다. 공간이 좁다는 이유로 `<AlertTitle>`을 생략하지 않는다.
...
- Alert 기본 컴포넌트: `app/front-end/src/components/ui/alert.tsx`
- Alert 기본 컴포넌트 Storybook: `app/front-end/src/components/ui/alert.stories.tsx`
- 회원 가입 양식 전체 오류: `app/front-end/src/features/signup/pages/SignupPage.tsx`
- 로그인 양식 전체 오류: `app/front-end/src/features/auth/pages/LoginPage.tsx`

### 입력 대화상자 생명주기

생성/수정처럼 목록 맥락 안에서 하나의 객체를 다루는 입력 양식은 대화상자 또는 드로어 안에 둘 수 있다. 입력 대화상자는 화면 전역 상태와 섞지 않고, 대화상자 인스턴스 안에서 입력·검증·제출 중·서버 오류 상태를 소유한다.

- 구조는 `Dialog` + `DialogContent` + `DialogHeader` + `DialogTitle` + `DialogDescription` + form + `DialogFooter`를 기본으로 한다.
- 입력 상태는 대화상자 본문 안의 하위 컴포넌트가 소유한다. 대화상자가 열릴 때 초기값으로 새로 시작하도록 본문을 remount하거나 key를 부여한다. 단순히 props 변화를 `useEffect`로 복사해 동기화하지 않는다.
- 생성과 수정이 같은 입력 양식을 공유하면 모드별 제목, 설명, 제출 버튼 문구만 달리하고 검증·오류 표시·제출 중 동작은 동일하게 유지한다.
- 브라우저 쪽 검증은 제출 전에 수행하고, 오류는 해당 입력 아래에 둔다. 이때 서버 요청을 보내지 않는다는 사실은 사용자 시나리오 문장이 아니라 테스트 assertion으로 검증한다.
- 제출 중에는 확인 버튼을 실제 `disabled` 상태로 만들고 `aria-busy`를 둔다. 중복 제출은 처리 함수와 비활성 상태 양쪽에서 막는다.
- 취소/닫기 버튼도 제출 중에는 비활성화할 수 있다. 닫기를 허용하는 경우에는 진행 중 요청의 취소 또는 결과 무시 정책을 코드와 테스트에서 명확히 한다.
- 성공하면 대화상자를 닫고 목록/상세 같은 서버 상태를 갱신한다. 성공 일시 알림은 요건에서 요구할 때만 둔다.
- 특정 입력에 귀속되지 않는 서버 오류는 위 "양식 전체 서버 오류 경고 안내" 구조로 대화상자 안에 남긴다. 사용자가 수정해야 하는 오류를 일시 알림으로만 표시하지 않는다.
- 서버가 입력 항목 오류를 반환하고 필드가 특정되면 해당 입력 아래 오류로 매핑한다. 원시 오류 코드 문자열은 사용자 문구로 그대로 노출하지 않는다.
- Storybook에는 최소 생성, 수정, 주요 입력 항목 오류, 제출 중, 대표 서버 거절 상태를 둔다. 성공 후 닫힘은 경로/화면 모의 story의 상호작용으로 검토할 수 있다.
- 각 Storybook 상태는 Docs 설명에 화면 목적, 입력 조건, 사용자 흐름, 관찰 포인트를 남긴다. 설명 작성 기준은 `front-end-testing.md`의 "Storybook Docs 설명"을 따른다.

참조 구현:

- Dialog 기본 컴포넌트: `app/front-end/src/components/ui/dialog.tsx`
- Dialog 기본 컴포넌트 Storybook: `app/front-end/src/components/ui/dialog.stories.tsx`
- 카테고리 생성/수정 Skeleton: `app/front-end/src/features/categories/components/CategoryFormDialog.tsx`
- 카테고리 생성/수정 Storybook: `app/front-end/src/features/categories/components/CategoryFormDialog.stories.tsx`
...
- 하단 영역에는 취소 버튼과 위험 강조 확인 버튼을 둔다. 확인 버튼 텍스트는 동작 동사로 쓴다. 예: "삭제", "취소 확정".
- 확인 요청 중에는 확인 버튼을 `disabled` + `aria-busy`로 표시하고 중복 실행을 막는다. 취소/닫기를 비활성화할지 여부는 요청 취소 가능성과 결과 처리 정책에 맞춘다.
- 성공하면 대화상자를 닫고 관련 서버 상태를 갱신한다.
- 실패하면 대화상자를 닫지 않고 양식 전체 서버 오류 경고 안내를 대화상자 안에 표시해 사용자가 재시도하거나 취소할 수 있게 한다.
- Storybook에는 최소 기본, 제출 중 상태를 둔다. 서버 실패를 화면에서 별도 처리하면 오류 상태도 둔다.
- 각 Storybook 상태는 Docs 설명에 확인 대상, 사용자 흐름, 제출 중/실패 시 관찰 포인트를 남긴다. 설명 작성 기준은 `front-end-testing.md`의 "Storybook Docs 설명"을 따른다.

참조 구현:

- AlertDialog 기본 컴포넌트: `app/front-end/src/components/ui/alert-dialog.tsx`
- AlertDialog 기본 컴포넌트 Storybook: `app/front-end/src/components/ui/alert-dialog.stories.tsx`
- 카테고리 삭제 확인 Skeleton: `app/front-end/src/features/categories/components/CategoryDeleteDialog.tsx`
- 카테고리 삭제 확인 Storybook: `app/front-end/src/features/categories/components/CategoryDeleteDialog.stories.tsx`
...
- Storybook Vitest/live E2E는 한 묶음을 넘는 데이터에서 스크롤 후 다음 항목이 보이는지로 검증한다. 가상화로 처음에는 일부만 DOM에 있으므로, 특정 항목 존재를 단정하기 전에 스크롤한다.
- 가상 스크롤 구현 세부(렌더 윈도 크기, 라이브러리 내부 속성)를 테스트에 고정하지 않는다.

참조 구현:

- 카테고리 목록 Skeleton: `app/front-end/src/features/categories/components/CategoryList.tsx`
- 카테고리 목록 Storybook: `app/front-end/src/features/categories/components/CategoryList.stories.tsx`
- 경로 화면 모의 story 상호작용: `app/front-end/src/features/categories/pages/CategoriesPage.stories.tsx`

Storybook Docs 설명:

- 목록 story는 데이터 개수와 정렬 기준, 항목에서 진입할 수 있는 동작, 빈 상태/로딩/오류/추가 묶음 존재 같은 상태를 설명한다.
- 경로/화면 모의 story는 사용자가 목록에서 생성/수정/삭제 흐름으로 이동하는 순서와 결과 반영 위치를 설명한다.
...
- 대화상자/드로어 안의 입력 양식도 열린 직후 첫 번째 입력에 자동 포커스를 둔다.
...
- 선택된 스와치는 `aria-pressed`와 포커스 표시 링으로 상태를 드러낸다. 색상만으로 선택 상태를 구분하지 않는다.
...
- 기능마다 팔레트와 허용 형식이 다를 수 있으므로 팔레트 값, 정규식, 최대/최소 정책은 해당 기능의 타입/상수에서 소유한다.
- Storybook에는 기본, 선택됨, 형식 오류, 비움 가능 상태를 검토할 수 있는 story를 둔다. 도메인 입력 양식 story에서 상태를 충분히 보여주면 별도 공통 기본 컴포넌트 story를 중복하지 않아도 된다.

참조 구현:

- 색상 상수와 검증 한계값: `app/front-end/src/features/categories/types.ts`
- 카테고리 색상 입력 Skeleton: `app/front-end/src/features/categories/components/CategoryFormDialog.tsx`
- 카테고리 색상 입력 Storybook: `app/front-end/src/features/categories/components/CategoryFormDialog.stories.tsx`

### 비밀번호 보이기/가리기 토글

- 모든 비밀번호 입력은 처음에 마스킹된 상태(`type="password"`)로 둔다.
- 입력 우측에 눈 모양 아이콘(`lucide-react`의 `Eye`/`EyeOff`) 토글 버튼을 둔다. 토글 버튼은 비밀번호 입력과 같은 입력 컨트롤 묶음 안에 두고, 레이블과 오류 메시지는 입력에 그대로 연결한다.
- 토글이 보이기 상태이면 입력 type을 `text`로 바꿔 입력값이 그대로 보이고, 가리기 상태로 되돌리면 다시 `password`로 마스킹한다.
- 토글 버튼은 키보드로 접근 가능해야 한다. 포커스 표시 상태를 유지하고, Tab 순서에서 비밀번호 입력 직후에 온다.
...
- 모든 입력·선택 조작 요소는 이름과 연결한다.
- 오류 메시지는 해당 입력과 연결한다.
- 포커스 표시 상태를 없애지 않는다.
- 아이콘만 있는 버튼은 접근 가능한 이름을 가진다.
- 색상만으로 상태를 구분하지 않는다.
- 비활성 상태는 시각적으로만이 아니라 실제 조작 불가 상태여야 한다.
- 키보드 탐색으로 주요 업무를 수행할 수 있어야 한다.

## Storybook

공통 컴포넌트는 Storybook story를 둔다. 경로/화면을 추가하거나 변경하면 실제 화면 컴포넌트를 `MemoryRouter`와 필요한 모의 provider로 감싼 경로 화면 모의 story를 둔다. Storybook story는 두 분류로 나눈다.

Storybook은 UI가 있는 요건의 Skeleton 단계 공식 검토 표면이다. 요건 카드의 `Storybook 계약`에 선언된 화면/컴포넌트/대화상자/목록 `title`과 이름 붙은 story 상태는 Skeleton 승인 전에 실제 story로 작성되어야 하며, `parameters.harness.requirements`로 해당 요건에 연결되어야 한다. `npm run build-storybook`은 이 검토 표면이 빌드 가능한지 확인하는 필수 Skeleton 게이트다.

### Router/provider 데코레이터는 하나만 둔다

경로/화면 모의 story는 `MemoryRouter`와 모의 provider로 감싸되, **Router 래퍼는 한 스토리의 렌더 트리에 정확히 하나만** 둔다. Storybook 데코레이터는 덮어쓰기가 아니라 합성(중첩)된다. 적용 순서는 바깥에서 안쪽으로 `global → meta(component) → story`이고, 스토리 레벨 데코레이터가 meta 데코레이터 **안쪽**에 들어간다. 따라서 meta 데코레이터에서 이미 `<MemoryRouter>`로 감쌌다면, 특정 스토리에서 진입 경로만 바꾸려고 스토리 레벨에 두 번째 `<MemoryRouter>` 데코레이터를 추가하면 Router가 중첩되어 react-router가 `You cannot render a <Router> inside another <Router>` 오류를 던진다.
...
위 상태 이름은 `*.stories.tsx`의 이름 붙은 story(`export const Default: Story = ...`)와 정확히 일치해야 한다. 대소문자, 공백 모두 일치를 요구한다. 변형이 필요하면 표준을 먼저 갱신한다.
...
Dialog: Default, Destructive, Loading
Table: Loading, Empty, Filled, Overflow
Pagination: First, Middle, Last, Disabled
```

`Input/Dialog/Table/Pagination`처럼 아직 만들어지지 않은 컴포넌트의 story를 미리 강제하지 않는다. 해당 컴포넌트가 실제로 추가되는 시점에 필수 상태를 본 절에 옮겨 정의한다.

Storybook은 화면 설계서 대체 산출물이며, `parameters.harness.covers`가 있는 Storybook Vitest story는 UI AC 완료 판정 입력이다. 화면 모의 story는 상위 통합 성과 검증을 대신하지 않으며, 여러 기능을 결합해야 하는 `(E2E)` AC는 live Playwright smoke로 확인한다.

## 금지 사항

- CSS selector나 class 이름에 업무 의미를 의존
- 모든 margin/padding/font-size를 테스트로 고정
- Storybook 없는 공통 UI 기본 컴포넌트 추가
- Storybook 없는 경로/화면 추가
- 접근 가능한 이름 없는 아이콘 버튼
- 한 화면 전체가 단일 색상 계열로만 읽히는 팔레트
- 장식용 gradient orb, bokeh blob 배경
- 비밀번호 입력에 보이기/가리기 토글 없이 마스킹만 두는 형태
...
- `npm run test:storybook` 또는 `npm run e2e`: 대표 화면의 데스크톱/접근성 smoke test 확인.
- (예정) 시각 회귀: 핵심 Storybook story와 주요 화면 캡처 기준선 비교.

## 수동 리뷰 항목

- 실제 업무 화면이 첫 화면에서 바로 사용 가능한가
- 공통 컴포넌트 상태가 Storybook에 충분히 표현되어 있는가
- 경로 기준 화면 모의 story가 실제 화면 컴포넌트와 필요한 모의 provider로 구성되어 있는가
- 데스크톱 기준 화면에서 텍스트와 컨트롤이 겹치지 않는가
- 디자인 변경이 token/variant로 흡수되는가
- 단일 폼 화면(로그인, 회원 가입 등)에서 첫 번째 입력 자동 포커스, 의미에 맞는 `autocomplete`, 비밀번호 보이기/가리기 토글이 표준대로 들어가 있는가
- 입력 대화상자가 열릴 때 입력/오류/제출 중 상태를 예측 가능하게 초기화하고, 성공·실패·닫기 정책이 명확한가
- 되돌릴 수 없는 동작 확인 대화상자가 대상, 결과, 취소, 확인, 제출 중, 실패 재시도 상태를 모두 표현하는가
- 선택 색상 입력이 색상만으로 상태를 전달하지 않고 스와치·직접 입력·지우기·형식 오류를 접근 가능하게 제공하는가
- 보호 앱 셸을 새 경로에 적용할 때 내비 활성 상태, 경로 story, 로그인 후 복귀 영향이 함께 검토되었는가
