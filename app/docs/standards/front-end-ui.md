...
Storybook은 화면 설계서 대체 산출물로 본다. 단, Storybook story 자체가 요건 완료 판정은 아니다. 완료 판정은 카드 AC와 BDD 테스트 커버리지로 한다. 화면 모의 story는 라우팅 검증을 대신하지 않으며, 실제 경로 진입과 사용자 흐름은 Playwright FE BDD 테스트로 확인한다.
