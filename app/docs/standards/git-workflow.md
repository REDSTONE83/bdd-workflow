...
브랜치·커밋·PR 규약을 둔다. 이 문서는 `app/docs/standards` 아래에 있지만 애플리케이션 구현 표준이 아니라 저장소 공통 협업 표준이다. 앱 작업과 하네스 작업 모두 PR 생성 또는 PR 본문 수정 전에 이 문서를 확인한다.
...
- **검증**: 실행한 게이트 결과를 그대로 적는다. 최소한 다음을 포함한다.
  ...
  - 영향 카드의 `npm run harness:trace -- --requirement REQ-XXX`
