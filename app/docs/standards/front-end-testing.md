...
프런트엔드 패키지에서 직접 실행하는 `npm run test:storybook`과 `npm run e2e`는 env가 없으면 Storybook Vitest canonical 결과 파일을 갱신한다. 루트 `npm run app:e2e`는 run root 결과를 생성한 뒤 성공 시 canonical 결과 파일을 publish한다. 프런트엔드 패키지에서 직접 실행하는 `npm run e2e:live`는 env가 없으면 live canonical 결과 파일을 갱신하고, 루트 `npm run app:e2e:live`는 run root 결과를 생성한 뒤 성공 시 canonical 결과 파일을 publish한다. 루트 `npm run app:validate`는 백엔드 Acceptance Test, Storybook Vitest, live smoke를 모두 실행한 뒤 trace/gate를 판정하고 성공한 실행만 canonical 결과로 publish한다.
