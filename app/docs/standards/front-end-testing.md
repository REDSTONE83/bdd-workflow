...
Storybook Vitest 실행 결과는 `app/front-end/test-results/storybook-junit.xml`로 남긴다. 상위 요건 live smoke 실행 결과는 `app/front-end/test-results/e2e-live-results.json`으로 남긴다. `npm run app:trace`와 `npm run app:validate`는 두 파일을 함께 읽어 프런트엔드 BDD 테스트 상태를 병합한다.

`npm run test:storybook`과 `npm run e2e`는 Storybook Vitest canonical 결과 파일을 갱신한다. `npm run e2e:live`와 루트 `npm run app:e2e:live`는 live canonical 결과 파일을 갱신한다. 루트 `npm run app:validate`는 백엔드 Acceptance Test, Storybook Vitest, live smoke를 모두 실행한 뒤 trace/gate를 판정한다.

원자 요건의 `(UI)` AC 검증 전에는 `npm run test:storybook` 또는 `npm run e2e`로 Storybook Vitest 전체 결과를 새로 갱신한다. 상위 요건의 live `(E2E)` AC 검증 전에는 `npm run e2e:live` 또는 루트 `npm run app:e2e:live`를 실행한다.

`npm run app:trace`는 테스트를 재실행하지 않고 디스크의 canonical 결과를 읽는다. 테스트 코드나 서버 동작을 바꾼 뒤에는 stale PASS를 피하기 위해 `npm run app:validate`, `npm run test:storybook`, 또는 `npm run e2e:live`로 해당 결과 파일을 먼저 갱신한다.
