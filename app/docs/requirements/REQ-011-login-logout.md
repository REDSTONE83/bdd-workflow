...
검증 수준: mixed
관련 요건: REQ-001, REQ-004, REQ-005, REQ-021
대체 요건: 없음
...
- 로그아웃 항목 선택 시 별도의 확인 대화상자 없이 즉시 `POST /auth/logout`을 호출한다. 성공 응답을 받았을 때에만 캐시된 사용자 정보를 비우고 `/login`으로 이동시킨다.
...
- `/signup` route는 본 카드가 가입 진입 링크용으로 처음 placeholder를 두었으나, REQ-001(이메일 회원 가입 화면)이 실제 화면으로 구현하면서 placeholder 책임을 이관했다. `/todos` route도 REQ-023(할 일 목록 조회)이 실제 본문을 구현하면서 본 카드의 placeholder 책임에서 제외된다. 로그인 성공 기본 진입점과 보호 라우트 인증 확인 책임은 본 카드에 남는다.
...
- 로그아웃 확인 대화상자
- 가입 화면(`/signup`) 본문 구현 (REQ-001 이메일 회원 가입 화면이 구현한다; 본 카드는 가입 진입 링크만 유지한다)
- 할 일 목록 화면(`/todos`) 본문 구현 (`REQ-023`과 할 일 원자 요건이 구현한다)

## 수용 기준

각 AC bullet 앞에 검증 채널 마커를 둔다. API/서비스/인증 계약은 `(API)`, 화면과 라우팅 표시는 `(UI)`, 로그인·보호 화면·로그아웃을 관통하는 사용자 여정은 `(E2E)`로 검증한다. 마커는 `@Covers`, FE `Covers`, `.feature` `Covers:` 값에 포함하지 않는다.

- (API) 등록된 이메일과 비밀번호로 로그인하면 인증 세션이 시작된다
- (API) 가입할 때 사용한 이메일과 대소문자와 앞뒤 공백만 다른 값으로 로그인해도 같은 계정으로 인증된다
- (API) 등록되지 않은 이메일로 로그인하면 이메일 존재 여부를 알 수 없는 동일한 인증 실패 응답을 받는다
- (API) 등록된 이메일이지만 비밀번호가 일치하지 않으면 동일한 인증 실패 응답을 받는다
- (API) 이메일 또는 비밀번호가 비어 있거나 이메일 형식이 아닌 로그인 요청은 인증을 시도하기 전에 형식 검증 오류 응답으로 거절되고, 자격 증명 인증 실패 응답과 구분된다
- (API) 운영 프로파일에서 로그인 성공 시 발급되는 인증 정보는 브라우저 JS가 직접 읽을 수 없고, HTTPS에서만 전송되며, 다른 사이트로부터의 자동 전송이 차단된다
- (API) 로컬과 테스트 프로파일에서 로그인 성공 시 발급되는 인증 정보는 브라우저 JS가 직접 읽을 수 없고 다른 사이트로부터의 자동 전송이 차단되며, HTTPS 강제 전송만 적용되지 않는다
- (API) 로그인 성공 응답의 본문은 비어 있다
- (API) 로그인 인증 정보의 유효 기간은 발급 시점부터 60분이다
- (API) 발급된 인증 정보가 있는 보호 API 요청은 인증된다
- (API) 인증 정보가 Cookie와 Authorization 헤더에 모두 있으면 Cookie의 인증 정보로 인증된다
- (API) 인증 정보가 Cookie에 없고 유효한 Authorization Bearer 헤더만 있으면 보호 API 요청이 인증된다
- (API) 인증 정보가 없으면 보호 API 요청이 거절된다
- (API) 로그아웃 호출이 성공하면 인증 세션이 종료되어 같은 클라이언트의 이후 보호 API 요청이 거절된다
- (API) 인증되지 않은 상태에서 로그아웃을 호출해도 인증 세션 종료와 동일한 응답을 받는다
- (API) 인증된 상태에서 현재 사용자 조회를 호출하면 자신의 식별자와 이메일이 반환된다
- (API) 인증되지 않은 상태에서 현재 사용자 조회를 호출하면 거절된다
- (API) OpenAPI 문서에는 Cookie 인증 방식과 Bearer 인증 방식이 모두 등록된다
- (API) 보호 API는 Cookie 인증 방식과 Bearer 인증 방식 중 하나로 호출할 수 있다고 OpenAPI 문서에 표시된다
- (API) 로그인과 로그아웃 API는 OpenAPI 문서에 인증 요구 없이 호출할 수 있다고 표시된다
- (UI) 로그인 화면은 화면 가운데에 하나의 로그인 카드를 표시하고, 카드는 이메일 입력, 비밀번호 입력, 로그인 버튼으로 구성된다
- (UI) 로그인 카드 하단에는 가입 화면으로 이동하는 텍스트 링크가 있다
- (UI) 로그인 화면을 열면 이메일 입력에 자동으로 입력 포커스가 간다
- (UI) 로그인 화면의 비밀번호 입력은 처음에는 입력값이 화면에 그대로 보이지 않게 가려진다
- (UI) 로그인 화면의 비밀번호 입력 옆에는 입력값을 보이거나 다시 가릴 수 있는 토글 버튼이 있다
- (UI) 비밀번호 토글을 보이기로 바꾸면 입력값이 화면에 그대로 보이고, 다시 가리기로 바꾸면 다시 가려진다
- (UI) 비밀번호 토글은 키보드로 조작할 수 있고, 현재 보이기와 가리기 상태가 보조 기술에 안내된다
- (UI) 이메일을 비워둔 채 로그인을 시도하면 이메일 입력 아래에 입력이 필요하다는 안내가 보인다
- (UI) 비밀번호를 비워둔 채 로그인을 시도하면 비밀번호 입력 아래에 입력이 필요하다는 안내가 보인다
- (UI) 이메일 형식이 아닌 값을 입력하고 로그인을 시도하면 이메일 입력 아래에 형식 안내가 보인다
- (UI) 키보드 Enter 입력으로 로그인을 제출할 수 있다
- (UI) 로그인 버튼을 누른 뒤 응답을 기다리는 동안 로그인 버튼은 다시 누를 수 없는 상태로 표시된다
- (UI) 로그인 응답을 기다리는 동안 같은 폼을 다시 제출해도 추가 로그인 요청이 서버로 전송되지 않는다
- (UI) 로그인 화면에서 인증에 실패하면 폼 상단에 어느 쪽이 잘못됐는지 구분하지 않는 공통 안내가 보이고, 입력했던 이메일은 그대로 유지되며 비밀번호 입력은 비워진다
- (E2E) 로그인 화면에서 인증에 성공하면 원래 가려고 했던 보호 화면이 있으면 그 화면으로, 없으면 자신의 할 일 목록 화면으로 이동한다
- (UI) 로그인 성공 후 이동 대상이 본 애플리케이션의 보호 라우트 경로가 아니거나 외부 사이트로 가는 값이면 그 값은 무시되고 할 일 목록 화면으로 이동한다
- (UI) 이미 인증된 사용자가 로그인 화면에 접근하면 할 일 목록 화면으로 이동한다
- (UI) 비인증 사용자가 보호 화면에 접근하면 로그인 화면으로 이동한다
- (E2E) 비인증 사용자가 보호 화면에 접근했다가 로그인에 성공하면 원래 가려고 했던 보호 화면으로 돌아온다
- (UI) 보호 화면을 처음 열거나 새로고침했을 때 인증 확인이 끝나기 전에는 헤더 골격과 본문 자리만 보이는 스켈레톤이 표시된다
- (UI) 보호 화면에는 공통 상단 헤더가 있고, 헤더 우측에 현재 사용자의 이메일이 표시된다
- (UI) 보호 화면 상단 헤더의 사용자 이메일을 선택하면 사용자 메뉴가 펼쳐지고, 그 안에 로그아웃 항목이 있다
- (E2E) 사용자 메뉴에서 로그아웃 항목을 선택해 로그아웃 호출이 성공하면 화면에서 사용자 정보가 사라지고 로그인 화면으로 이동한다
- (UI) 사용자 메뉴에서 로그아웃 항목을 선택해 로그아웃 호출이 실패하면 현재 화면에 그대로 머무르고 상단에 재시도를 안내하는 오류 표시가 노출된다
...
- 결정일: 2026-05-23
  결정: 모든 보호 화면은 공통 상단 헤더를 가지고, 사용자 메뉴는 헤더 우측의 사용자 이메일 dropdown 안에 둔다. 로그아웃 항목 클릭은 별도 확인 대화상자 없이 즉시 실행한다.
...
- 결정일: 2026-05-23 (2026-06-02 현행 마커로 갱신)
  결정: AC 단위 검증 채널을 카드에 명시한다. 수용 기준 bullet 앞에 `(API)`, `(UI)`, `(E2E)` 마커를 두고, 각각 백엔드 Acceptance Test, FE BDD 테스트, Playwright 사용자 여정 테스트 커버를 요구한다.
  이유: 본 카드는 API 계약과 UI 동작과 사용자 여정이 한 canonical 기능 안에 함께 있다. 카드를 API/화면으로 쪼개면 로그인과 로그아웃의 의사결정 흐름이 분산되므로, AC 단위 검증 채널로 정확한 커버 요구를 표현한다.
  결정자: Product Owner, Tech Lead
  영향: 본 카드의 모든 AC는 마커를 갖는다. 마커 파싱과 게이트는 REQ-012가 표준화했고, `evaluate-trace-state.mjs`의 `effectiveCoveragePolicy`가 `(API)`는 백엔드 Acceptance Test, `(UI)`와 `(E2E)`는 Playwright FE BDD 테스트를 `requiredChecks`로 강제한다. 본 카드의 RED/GREEN/BLUE 판정은 AC 마커별 커버 요구를 그대로 따른다.
...
- 결정일: 2026-05-30
  ...
  이유: 기존 단일 `AlertDescription` 평서문은 시각 위계가 약하고 다음 행동 안내가 없다. REQ-001 회귀 적용과 함께 새 표준이 도입돼 모든 form-level 서버 오류 Alert 구조를 통일한다. 인증 실패 본문은 어느 쪽 자격 증명이 잘못됐는지 노출하지 않는다는 본 카드 결정 #5 (user enumeration 보호)를 그대로 따른다.
  결정자: Tech Lead
...
- 결정일: 2026-05-30
  결정: `/signup` placeholder 책임을 REQ-001(이메일 회원 가입 화면)으로 이관한다. 본 카드의 `/signup` placeholder 수용 기준("`/signup` 경로에 접근하면 가입 화면이 아직 준비 중이라는 안내가 표시된다"), 해당 시나리오, Playwright placeholder 테스트, `SignupPlaceholderPage`/그 story 를 제거한다. 함께, REQ-001 가입 성공 흐름을 위해 `LoginPage` 에 `signupCompleted` 쿼리 감지 진입점을 추가한다.
  이유: REQ-001 이 `/signup` 을 실제 회원 가입 화면으로 구현하면서, placeholder 기대가 그대로 남으면 새 화면 테스트와 REQ-011 placeholder 테스트가 같은 route 에 대해 상충한다. REQ-001 의사결정 로그(2026-05-27 "REQ-011 placeholder 기대 이관")가 이 정리를 본 카드 구현 단계 또는 REQ-001 구현 단계에서 수행하도록 예고했고, REQ-001 구현 단계에서 함께 정리한다.
  결정자: Tech Lead
  영향: 본 카드의 UI 수용 기준이 23개에서 22개로 줄어 API 20 + UI 22 + E2E 3 = 45개가 된다(시나리오 37개). 로그인 카드의 `/signup` 진입 링크는 그대로 유지되며 이제 실제 회원 가입 화면으로 이동한다. `LoginPage` 의 `signupCompleted` 안내 표시는 본 카드 파일에 구현되지만, 그 동작("`/login`으로 이동하면 가입이 완료되었다는 안내가 보인다")은 REQ-001 의 수용 기준이며 REQ-001 의 성공 흐름 Playwright 테스트가 커버한다. 본 카드는 새 AC 를 추가하지 않는다. 이관 후에도 본 카드의 나머지 45개 AC 는 모두 커버되어 BLUE 를 유지한다.

- 결정일: 2026-06-06
  결정: `/todos` placeholder 책임을 REQ-023(할 일 목록 조회)과 할 일 원자 요건으로 이관한다. 본 카드의 `/todos` placeholder 수용 기준("인증된 사용자가 `/todos` 경로에 접근하면 자신의 이메일이 표시되는 빈 보호 화면이 보인다"), 해당 시나리오, Playwright placeholder 메타데이터, `TodosPlaceholderPage`/그 story 를 제거한다.
  이유: 할 일 목록 조회와 생성/수정/삭제/완료 상태 변경 원자 요건이 `/todos`를 실제 할 일 관리 화면으로 구현하면서 빈 본문 placeholder 기대와 새 화면 기대가 같은 route에서 상충한다.
  결정자: REDSTONE
...
- 2026-05-27 BE Skeleton 승인 (REDSTONE).
  ...
  - 표준 게이트 보완: `AuthService` 공개 메서드 3개에 `@Transactional`/`@Transactional(readOnly = true)` 부여. 하네스 BE-C3 list 휴리스틱을 `/me`·`/current`·`/self` singleton fetch 예외로 확장(`harness/tools/validate-back-end-standards.mjs`).
...
- 리뷰일: 2026-05-27
  ...
  확인: API 20 + UI 22 + E2E 3 = 45개 수용 기준이 모두 `@Covers` 또는 Playwright `testInfo.annotations Covers`와 `.feature` `Covers:` 블록에 정확 일치로 매핑되었고, `traceRequirementCard -Preq=REQ-011`/`validateRequirementCard -Preq=REQ-011`/`npm run validate` 모두 PASS. BE Acceptance Test 23/23, Playwright BDD 22/22, 게이트 `red=0 green=0 blue=12`. 카드/표준/시나리오/용어 finding 0건. open redirect 화이트리스트, profile 분기 Cookie Secure, Cookie/Bearer 우선순위, E2E 3개 사용자 여정 커버까지 확인했다.
  결과: 승인
