...
- 화면에서는 수정 모달로 제목, 설명, 마감일, 우선순위, 카테고리를 수정한다.
...
- 결정일: 2026-06-06
  ...
  이유: 수정 모달의 기존값 표시, 선택 정보 비우기, 저장 실패 재시도는 수정 기능의 사용자 완료 조건이며 API PATCH 계약과 함께 추적되어야 한다.
  결정자: REDSTONE
  영향: 수정 모달, 수정 입력 검증, 수정 실패 재시도, 목록 반영 UI AC와 FE BDD 커버리지를 본 카드로 이동한다.
...
- 승인일: 2026-06-02
  ...
  화면/라우팅 Skeleton: 2026-06-06 화면 도입 후 `/todos` 화면의 할 일 수정 모달이 본 카드 UI AC를 커버한다.
  검사기 Skeleton: 해당 없음.
