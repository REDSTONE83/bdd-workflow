...
- 결정일: 2026-06-06
  ...
  이유: 사용자가 보는 생성 모달, 입력 검증, 성공 후 목록 반영은 생성 기능의 완료 조건이며 API 계약과 분리하면 추적과 승인 단위가 나뉜다.
  결정자: REDSTONE
  영향: 새 할 일 만들기 모달, 생성 입력 검증, 생성 실패 재시도, 목록 반영 UI AC와 FE BDD 커버리지를 본 카드로 이동한다.
...
- 승인일: 2026-06-02
  ...
  화면/라우팅 Skeleton: 2026-06-06 화면 도입 후 `/todos` 화면의 새 할 일 만들기 모달이 본 카드 UI AC를 커버한다.
  검사기 Skeleton: 해당 없음.
