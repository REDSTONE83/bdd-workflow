...
Feature: 이메일 회원 가입
  ...
  Scenario: 처음 가입하는 사용자가 정상적으로 계정을 만든다
    ...
    When 사용자가 이름, 이메일, 8자 이상 비밀번호로 가입한다
    Then 계정이 생성된다
    And 사용자는 가입된 계정의 이메일과 이름을 확인할 수 있다
  ...
  Scenario: 이미 가입된 이메일로 다시 가입을 시도해 실패한다
    ...
    When 다른 사용자가 같은 이메일과 유효한 비밀번호로 가입하려고 한다
    Then 가입이 거절되고 이미 가입된 이메일이라는 안내가 보인다
  ...
  Scenario: 비밀번호가 짧아 가입을 거절당한다
    ...
    Given 이메일 hong@example.com 은 아직 가입되어 있지 않다
    When 사용자가 8자 미만 비밀번호로 가입하려고 한다
    Then 가입이 거절되고 비밀번호가 너무 짧다는 안내가 보인다
    And 계정이 저장되지 않는다
