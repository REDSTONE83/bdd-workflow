...
- `@Requirement`는 클래스 또는 메서드에 명시한다 (`docs/standards/api-contract.md`).
...
- Entity 작성 규칙은 `docs/standards/persistence-schema.md`를 따른다.
...
- 전역 HTTP 매핑은 `common/ApiExceptionHandler.java`에서 처리한다 (`docs/standards/api-contract.md`).
...
- 새 도메인을 추가할 때 위 6개 하위 패키지 외 임의 패키지명을 만들지 않는다. 필요한 경우 표준 갱신 절차(`docs/standards/README.md`)를 거친다.

## 자동 검증 항목

- `generateHarnessSourceIndex`: 도메인별 클래스 위치를 source index로 수집한다.
