...
브랜치·커밋·PR 규약을 둔다. 이 표준은 하네스 자동 검증 대상이 **아니다**(게이트 입력이 아님). PR 리뷰에서 수동으로 확인한다. 요건 완료 판정은 여전히 카드 수용 기준 커버리지와 `./gradlew validateHarness`로 한다.
...
- 생성 산출물(`build/`, `front-end/dist`, `storybook-static`, `test-results`, `playwright-report`)은 커밋하지 않는다(`.gitignore` 유지).
...
- **검증**: 실행한 게이트 결과를 그대로 적는다. 최소한 다음을 포함한다.
  - `./gradlew validateHarness` 게이트 결과와 RED/GREEN/BLUE 요약
  - 영향 카드의 `./gradlew traceRequirementCard -Preq=REQ-XXX`
...
- 모든 게이트가 통과한 상태(`validateHarness` exit=0, 영향 카드가 BLUE이거나 카드에 명시된 의도된 상태)에서만 머지한다.
