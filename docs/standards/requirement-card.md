...
`구현 대상`이 `front-end` 또는 `full-stack`인 카드에서 화면이 범위에 포함되면, 화면 레이아웃과 카드 구성을 페이징과 동일한 원칙으로 다룬다. 사용자가 보는 표시 단위가 완료 판정에 직접 영향을 주므로 `범위`에는 화면 레이아웃 단위(예: 보호 화면 chrome 사용, 비인증 중앙 단일 카드)와 카드의 주요 표시 요소(제목, 입력, 버튼, 링크, 안내 영역 등)를 narrative로 적고, `수용 기준`에는 그 구성이 화면에 표시된다는 사실을 검증 가능한 한 줄로 별도 AC로 둔다. `범위`에만 적힌 화면 구성은 FE BDD 테스트로 커버되지 않아 사용자가 보는 표시가 의도와 달라져도 RED로 잡히지 않는다.

작성 예 (`front-end` 카드):

```text
범위
- 회원 가입 화면은 비인증 단일 카드 레이아웃을 사용하고, 카드는 제목, 사용자 이름 입력, 이메일 입력, 비밀번호 입력, 회원 가입 버튼, 로그인 화면으로 돌아가는 링크로 구성한다

수용 기준
- (FE) 회원 가입 화면은 화면 가운데에 하나의 회원 가입 카드를 표시하고, 카드는 제목, 사용자 이름 입력, 이메일 입력, 비밀번호 입력, 회원 가입 버튼, 로그인 화면으로 돌아가는 링크로 구성된다
```

입력 UX 세부(자동 포커스, autocomplete 속성, 비밀번호 show/hide 토글, 키보드 제출 등)는 본 카드 AC로 둘지 FE 공통 표준([`front-end-ui.md`](./front-end-ui.md))에 위임할지를 `의사결정 로그`로 확정한다. 본 카드 AC에 두기로 한 항목만 별도 수용 기준으로 적고, 위임한 항목은 카드 AC에 복제하지 않는다. 자세한 질문 절차는 [`docs/harness/requirement-authoring.md`](../harness/requirement-authoring.md) "화면 레이아웃과 카드 구성"에 둔다.
...
요건에 실행 환경 경계, 생명주기 대칭, 외부 계약 노출, 불신 입력, 실패 분류가 포함되면 해당 정책을 카드의 자연어 어딘가에 흩어 두지 말고, 다음 위치 중 하나에 명시적으로 귀속한다. 자세한 점검 질문은 [`docs/harness/requirement-authoring.md`](../harness/requirement-authoring.md) "공통 리스크 패턴 점검"에 둔다.

- 실행 환경 경계: 환경별 동작은 `범위`에 쓰고, 왜 환경별로 다른지는 `의사결정 로그`에 남긴다. 환경별 결과가 사용자나 외부 클라이언트에 관찰되면 `수용 기준`도 분리한다.
- 생명주기 대칭: 생성, 발급, 저장, 시작의 정리·만료·해제 정책은 `범위`나 `제외 범위`에 둔다. 정리 결과가 완료 판단이면 별도 `수용 기준`으로 둔다.
- 외부 계약 노출: API, route, OpenAPI, 이벤트, 파일 형식, 화면 진입점처럼 외부가 의존하는 계약은 `범위`에 계약 방향을 쓰고, 호환성이나 공개/보호 표시가 완료 판단이면 `수용 기준`으로 둔다.
- 불신 입력: query, redirect target, callback URL, filter, sort, path, 업로드 값의 허용 범위와 fallback 또는 거절 정책은 `범위`와 `의사결정 로그`에 둔다. 사용자가 관찰하는 결과는 `수용 기준`으로 둔다.
- 실패 분류: 검증 실패, 업무 규칙 위반, 인증/권한 실패, 부재, 충돌, 일시 실패를 같은 결과로 숨길지 구분할지는 `의사결정 로그`에 남기고, 테스트 가능한 결과는 `수용 기준`으로 둔다.
...
- `화면/라우팅 Skeleton`: 별도 파일로 작성한 인터랙션 mockup 컴포넌트 위치, 예상 route 초안 경로, 페이지 컴포넌트 이름, 접근 권한, 주요 표시 정보, 작성한 route 기준 page mock story와 상태별 story 위치(initial / fieldErrors / submitting / serverRejection / success 등). routes.tsx swap, 외부 API client 결합, 다른 카드 placeholder 정리, `@Covers` FE BDD 테스트, visual snapshot baseline은 구현 단계 작업 목록으로 남기고 Skeleton에서는 작성하지 않는다. (`구현 대상: front-end`/`full-stack`)
...
- 실행 환경 경계, 생명주기 대칭, 외부 계약 노출, 불신 입력, 실패 분류 중 해당되는 패턴이 `범위`/`제외 범위`/`수용 기준`/`의사결정 로그`에 귀속되었는가
- 목록 조회 API가 있으면 페이징 수용 기준이 별도로 포함되어 있는가
- 화면이 범위에 있으면 화면 레이아웃 단위와 카드 구성이 `범위` narrative와 검증 가능한 `수용 기준` 양쪽에 표현되어 있는가
- 열린 질문이 모두 닫혔는가 (승인 전)
