...
기존 카드에서 `구현 대상`이 생략되면 `back-end`로 본다. 하네스는 이 값을 RED/GREEN/BLUE 판정에 사용한다. `front-end`는 FE 화면/route/story 연결과 FE BDD 테스트 PASS가 필요하고, `full-stack`은 백엔드 API/Acceptance Test와 FE 화면/BDD 테스트가 모두 필요하다.
...
- API 목록이나 테스트 메서드 목록을 카드에 직접 적지 않는다. 코드의 `@Requirement`, `@Covers` 스캔으로 자동 생성된 리포트(`build/harness/trace-report.md`)에서 확인한다.
