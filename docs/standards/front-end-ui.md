...
- 모바일 가로 스크롤이 생기지 않는가
- 버튼과 입력이 화면 밖으로 나가지 않는가
- 테이블은 스크롤 또는 카드형 표현으로 안전하게 전환되는가
- 모달과 drawer가 화면 안에 들어오는가
- 헤더/사이드바가 축약될 때 주요 업무 진입점이 사라지지 않는가
...
공통 컴포넌트는 Storybook story를 둔다.

필수 상태 예:

```text
Button: Default, Secondary, Outline, Disabled, Loading, WithIcon
...
- `npm run e2e`: 대표 화면의 반응형/접근성 smoke test 확인.
...
- 모바일/태블릿/데스크톱에서 텍스트와 컨트롤이 겹치지 않는가
