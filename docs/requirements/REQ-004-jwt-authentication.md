...
품질 속성: security
검증 수준: acceptance
관련 요건: REQ-001, REQ-011, REQ-015, REQ-021
대체 요건: 없음
...
- 본 카드는 애플리케이션 보호 API에 공통 적용되는 JWT Bearer 인증 정책을 소유한다.
- 기존 REQ-002/REQ-003의 임시 사용자 헤더 기반 인증 정책은 본 카드로 대체되었고, 현재 canonical 기능 요건은 REQ-021/REQ-015이다.
...
- (API) 유효한 JWT Bearer 토큰이면 보호 API 요청이 인증된다
- (API) 인증된 사용자의 식별자는 JWT sub 클레임의 UUID로 결정된다
- (API) Authorization 헤더가 없으면 보호 API 요청이 거절된다
- (API) Authorization 헤더가 Bearer 형식이 아니면 보호 API 요청이 거절된다
- (API) JWT 형식이 잘못되면 보호 API 요청이 거절된다
- (API) JWT 서명 검증에 실패하면 보호 API 요청이 거절된다
- (API) HS256 외 알고리즘으로 서명된 JWT Bearer 토큰이면 보호 API 요청이 거절된다
- (API) JWT exp가 현재 시각 기준 60초 이내 과거이면 보호 API 요청이 인증된다
- (API) JWT exp가 현재 시각 기준 60초보다 더 오래전에 만료되었으면 보호 API 요청이 거절된다
- (API) JWT issuer가 서버 설정과 다르면 보호 API 요청이 거절된다
- (API) JWT audience가 서버 설정과 다르면 보호 API 요청이 거절된다
- (API) JWT sub 클레임이 없거나 UUID 형식이 아니면 보호 API 요청이 거절된다
- (API) 인증 실패 응답은 401 상태와 UNAUTHORIZED 오류 코드를 가진 ApiError 형식이다
- (API) X-User-Id 또는 X-Authenticated-User-Id 헤더만 전달된 요청은 Authorization 헤더 없는 요청과 동일하게 거절된다
- (API) 보호 API는 인증된 사용자의 자원만 생성, 조회, 수정, 삭제한다
- (API) 회원 가입 API는 JWT Bearer 토큰 없이 호출할 수 있다
- (API) OpenAPI 문서와 Swagger UI는 JWT Bearer 토큰 없이 조회할 수 있다
- (API) 공개 경로에 유효한 JWT Bearer 토큰이 전달되어도 요청이 허용된다
- (API) 공개 경로라도 잘못된 Bearer 토큰이 전달되면 요청이 거절된다
- (API) 보호 API 응답에는 서버 세션 쿠키가 포함되지 않는다
- (API) OpenAPI 문서에는 JWT Bearer SecurityScheme이 포함된다
...
- 결정일: 2026-06-02
  결정: REQ-004는 새 ID로 분리하지 않고 기존 ID를 유지한 채 `정책` / `원자 요건` 카드로 정리한다.
  이유: JWT 기반 인증은 특정 기능의 생성/조회/수정/삭제 생명주기가 아니라 여러 보호 API에 공통 적용되는 최종 인증 정책이며, 기존 AC와 테스트가 이미 단일 정책으로 정합하게 묶여 있다.
  결정자: Product Owner, Tech Lead
  영향: 카드 헤더를 새 스키마로 보강하고 모든 AC에 `(API)` 마커를 부여한다. Scenario `Covers:`와 실행 테스트 `@Covers` 문장은 마커 없이 기존 문장을 유지한다. 기존 승인된 동작과 검증 범위는 바꾸지 않는다.

## BDD 테스트 리뷰

- 시나리오 문서: `docs/scenarios/REQ-004-jwt-authentication.feature`
