...
- Change Set: 사용자 요청을 처리하기 위해 함께 바꾸는 REQ 집합. 영구 명세 원천이 아니라 작업 범위이며, 별도 사람이 관리하는 ID 없이 `docs/change-sets/*.md` 파일 경로를 identity로 쓴다.
