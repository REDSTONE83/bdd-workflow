...
목록 조회가 범위에 있으면 페이징 수용 기준을 반드시 작성한다. `PageResponse<T>` 엔벨로프 존재만으로 충분하지 않다. `page`/`size` 요청값에 따라 `content`가 잘리는지, `totalElements`와 `totalPages`가 전체 결과 기준으로 유지되는지 관찰 가능한 문장으로 적는다. 정렬이 있는 목록은 페이징 전에 적용되는 정렬 순서도 별도 수용 기준으로 둔다.
