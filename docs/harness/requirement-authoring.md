...
- 실행 테스트: 승인된 검증 설계를 JUnit Acceptance Test로 옮긴 코드. 요건 Skeleton 승인 후 작성한다.
- 업무 구현: Service 업무 로직과 Controller 성공 응답 본문을 작성하는 단계다.
- 하네스 검증: `validateHarness`로 AC, 시나리오, 테스트, API, Entity 연결과 테스트 결과를 확인하는 단계다.
...
요건 Skeleton은 별도 문서가 아니라 코드 골격(Controller/DTO/Entity/Repository/Service)과 `previewSchema` 산출물로 둔다. `.feature` 파일은 시나리오 본문에만 집중하고, API/DB/Service 명세는 평소대로 `back-end/src/main/java`의 골격 코드에서 확인한다. 작성 기준은 [`api-contract.md`](../standards/api-contract.md), [`persistence-schema.md`](../standards/persistence-schema.md), [`package-structure.md`](../standards/package-structure.md)를 따른다.
...
승인된 검증 설계의 `.feature` 시나리오 `Covers:`에 포함된 AC를 검증하는 테스트 메서드를 작성한다.
