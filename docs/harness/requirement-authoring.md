...
요건 카드는 단순 요청 기록이 아니라, 구현 전에 모호성을 줄이고 의사결정을 남기는 기준 문서다. 이 하네스에서는 요건 카드의 `수용 기준`이 Acceptance Test의 `@Covers` 또는 FE BDD 테스트의 `Covers` 메타데이터와 연결되어 구현 완료 여부를 판단한다.
...
수용 기준이 확정되면 시나리오 하나씩 승인받지 않는다. 요건 하나를 작업 단위로 삼아 검증 설계와 요건 Skeleton을 한 번에 세우고 사용자 승인을 받은 뒤, 같은 요건을 구현한다. Skeleton 단계는 인터페이스와 계약을 검토하는 단계이므로 업무 로직, 성공 응답 본문, Repository 쿼리 계약, 실행 테스트는 작성하지 않는다. 동작 내용은 Service/Controller 내부 코멘트로만 설계해 둔다.
...
- 하네스 검증: `validateHarness`와 FE 검증 명령으로 AC, 시나리오, 테스트, API, Entity, 화면 연결과 테스트 결과를 확인하는 단계다.
...
- 권한이나 인증이 필요한가?
- 화면 또는 라우팅이 필요한가?
- 모바일/태블릿/데스크톱에서 반드시 보장해야 할 표현이 있는가?
...
- 반응형: 모바일에서 테이블/사이드바/모달이 어떤 방식으로 전환되어야 하는가?
...
시나리오는 PO/QA/기획자/프론트엔드/백엔드가 함께 읽을 수 있는 업무 언어 단위이며, 원본은 Gherkin `.feature` 파일로 `docs/scenarios/REQ-XXX-*.feature`에 둔다. 요건 1건당 `.feature` 파일 1개를 두고, 그 요건의 수용 기준을 검토하는 시나리오 묶음을 한 번에 작성한다.
...
요건 Skeleton은 별도 문서가 아니라 코드 골격(Controller/DTO/Entity/Repository/Service), 화면/라우팅 Skeleton, `previewSchema` 산출물로 둔다. `.feature` 파일은 시나리오 본문에만 집중하고, API/DB/Service 명세는 평소대로 `back-end/src/main/java`의 골격 코드에서 확인한다. FE 대상 요건은 화면 이름, 업무 진입점, route 초안, 접근 권한, 주요 표시 정보, 필요한 Storybook 상태만 카드의 Skeleton 승인 이력에 남긴다. 작성 기준은 [`api-contract.md`](../standards/api-contract.md), [`persistence-schema.md`](../standards/persistence-schema.md), [`package-structure.md`](../standards/package-structure.md), [`front-end-project-structure.md`](../standards/front-end-project-structure.md), [`front-end-ui.md`](../standards/front-end-ui.md)를 따른다.
...
- 검증 설계: `docs/scenarios/REQ-XXX-*.feature` 시나리오 문서 (요건 전체의 Feature/Scenario/`Covers:`/Given/When/Then)
...
- Service: 인터페이스나 클래스 시그니처만 작성한다. 클래스 본문이 필요하면 내부 코멘트로 흐름을 적고 `throw new UnsupportedOperationException(...)`으로 닫는다.
...
Skeleton 단계의 카드는 아직 `@Covers` 테스트가 없으므로 RED로 잡힌다. 이는 정상이다. 이 시점에는 strict 게이트인 `validateHarness`를 돌리지 않는다. 다음 명령으로 컴파일, 소스 인덱스, 스키마, 추적 현황만 확인한다.

```bash
cd back-end
./gradlew compileJava                                # Skeleton 코드 컴파일 확인
./gradlew generateHarnessSourceIndex                 # Java source index 생성
...
FE 테스트 계층은 [`front-end-testing.md`](../standards/front-end-testing.md)를 따른다. 하네스 인덱서 도입 전까지 FE BDD 테스트의 `REQ`/`Covers` 메타데이터는 예정 형식으로 남기고, 카드 테스트 리뷰에는 `npm run validate:full` 결과와 수동 확인을 기록한다.
...
- FE 대상 요건이면 Playwright 테스트가 실제 사용자 행위, 주요 화면 표시, 권한/라우팅, 반응형 또는 접근성 요구를 필요한 만큼 검증하는가?
...
- FE 대상 요건이면 `npm run validate:full`이 성공한다.
