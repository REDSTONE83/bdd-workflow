...
`구현 대상`이 `front-end` 또는 `full-stack`인 카드에서 화면이 범위에 포함되면, 화면 레이아웃과 카드 구성을 페이징과 같은 원칙으로 다룬다. 사용자에게 어떤 표시 단위가 보이는지가 완료 판정에 직접 영향을 주기 때문이다.
...
3. 같은 구성을 `수용 기준`에 검증 가능한 한 줄로 별도 AC로 둔다. 예: "(FE) 회원 가입 화면은 화면 가운데에 하나의 회원 가입 카드를 표시하고, 카드는 제목, 사용자 이름 입력, 이메일 입력, 비밀번호 입력, 회원 가입 버튼, 로그인 화면으로 돌아가는 링크로 구성된다."
...
화면 카드(`구현 대상: front-end`/`full-stack`)는 위 화면 레이아웃 AC 외에 다음을 Skeleton 승인 전에 한 줄씩 확인한다. 정적 룰이 아니라 작성자·리뷰어의 공통 사고 틀이며, 실제 리뷰에서 반복적으로 누락된 항목을 모았다.
...
요건 Skeleton은 별도 문서가 아니라 코드 골격(Controller/DTO/Entity/Repository/Service), 화면/라우팅 Skeleton, `previewSchema` 산출물로 둔다. `.feature` 파일은 시나리오 본문에만 집중하고, API/DB/Service 명세는 평소대로 `back-end/src/main/java`의 골격 코드에서 확인한다. FE 대상 요건은 화면 이름, 업무 진입점, route 초안, 접근 권한, 주요 표시 정보, 필요한 Storybook 상태만 카드의 Skeleton 승인 이력에 남긴다. 작성 기준은 [`api-contract.md`](../standards/api-contract.md), [`persistence-schema.md`](../standards/persistence-schema.md), [`package-structure.md`](../standards/package-structure.md), [`transaction.md`](../standards/transaction.md), [`configuration.md`](../standards/configuration.md), [`front-end-project-structure.md`](../standards/front-end-project-structure.md), [`front-end-ui.md`](../standards/front-end-ui.md)를 따른다.
...
./gradlew generateFrontEndSourceIndex                # FE source index 생성
