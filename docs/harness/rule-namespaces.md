...
| `CHANGE-SET-*` | Change Set 작업 범위 문서 구조 검사 (report-only, 게이트 차단 없음) | `tools/harness/index-change-sets.mjs`, `tools/harness/render-change-set-report.mjs` | `change-sets.index.json` + `requirements.index.json` |
