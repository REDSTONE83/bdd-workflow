...
`docs/terminology/`에 정의된 term key만 적는다. 검색은 `node tools/harness/terminology.mjs search <표현>`을 사용한다. 새 용어가 필요하면 `node tools/harness/terminology.mjs draft add ...`로 후보를 등록한 뒤 key를 적는다 (`draft.json`은 직접 편집하지 않는다).
