...
// build/harness/state/trace.state.json 으로 출력한다.
//
// 입력:
//   indexes/*.{json}
//   findings/{requirement-cards,cross-artifact}.findings.json
//   (선택) build/harness/findings/terminology.findings.json
//
// CLI:
...
//
// 출력: build/harness/state/trace.state.json
...
function readAllRequirementCards() {
    if (!fs.existsSync(requirementsIndexPath)) {
        throw new Error(
            ...
            'Run node tools/harness/index-requirements.mjs first (e.g. via ./gradlew indexRequirements).'
        );
    }
