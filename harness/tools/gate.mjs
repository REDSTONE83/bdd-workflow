...
function loadFindings(file) {
    ...
    if (!fs.existsSync(full)) {
        console.error(`Missing required findings file: ${full}\nRun the matching Layer 2 validator first (see harness/docs/data-contracts.md).`);
        process.exit(2);
    }
...
function assertRequiredInputs() {
    ...
    if (missing.length > 0) {
        ...
        console.error('Run the matching Layer 2 validator first (see harness/docs/data-contracts.md).');
        process.exit(2);
    }
}
