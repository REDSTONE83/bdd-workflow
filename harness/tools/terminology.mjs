...
import { currentScope, outputRootFor, requirementsDirFor, terminologyDirFor, workspaceRoot } from './workspace-config.mjs';

const repoRoot = workspaceRoot;
const scope = currentScope();
const terminologyDir = terminologyDirFor();
...
const requirementsDir = requirementsDirFor();
...
function commandSearch(rawQuery) {
    if (!rawQuery || rawQuery.trim() === '') {
        console.error('Usage: node harness/tools/terminology.mjs search <query>');
        process.exit(2);
    }
    const loaded = loadTerminology();
...
function commandValidate(options) {
    ...
    if (!fs.existsSync(sourceIndexPath) && scope !== 'harness') {
        console.error(`Missing ${relativeToRepo(sourceIndexPath)}. 먼저 npm run app:source-index 를 실행하라.`);
        process.exit(1);
    }
    const sIndex = fs.existsSync(sourceIndexPath)
        ? JSON.parse(fs.readFileSync(sourceIndexPath, 'utf8'))
        : { textChannels: [], apis: [], entities: [], dtos: [], tests: [] };
    ...
    try {
        switch (sub) {
            ...
            case 'delete':
                ...
                break;
            default:
                console.error('Usage: node harness/tools/terminology.mjs draft <add|update|delete> [args]');
...
switch (command) {
    ...
    case 'draft':
        ...
        break;
    default:
        console.error('Usage: node harness/tools/terminology.mjs <index|search|validate|draft> [args]');
