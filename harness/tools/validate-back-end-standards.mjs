...
import { backendRoot, currentScope, outputRootFor, workspaceRoot } from './workspace-config.mjs';
...
if (scope === 'harness' || !fs.existsSync(sourceIndexPath)) {
    ...
    fs.writeFileSync(reportMd, '# Back-end Standards Findings (BE-*)\n\n- error: 0\n- warning: 0\n- total: 0\n');
    console.log(`back-end-standards.findings.json: 0 finding(s) (${scope === 'harness' ? 'harness scope' : 'source index missing'}) → ${relPath(findingsJson)}`);
    process.exit(0);
}
...
// 데이터 계약(harness/docs/data-contracts.md)에 맞춰 finding을 정규화한다.
...
const normalizedFindings = findings.map((f) => ({
    ...
    remediation: 'app/docs/standards/'
}));
