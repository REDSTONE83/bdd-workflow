...
//   indexes/harness.self-test.index.json
...
import { currentScope, outputRootFor, workspaceRoot } from './workspace-config.mjs';
...
function collectKnownRequirementIdsFromDocs() {
    const dirs = [
        path.join(repoRoot, 'app', 'docs', 'requirements'),
        path.join(repoRoot, 'harness', 'docs', 'requirements')
    ];
    const ids = new Set();
    for (const dir of dirs) {
        for (const file of walk(dir, (candidate) => candidate.endsWith('.md'))) {
            const content = fs.readFileSync(file, 'utf8');
            const id = content.match(/^요건 ID:[ \t]*(REQ-\d{3,})\s*$/m)?.[1];
            if (id) ids.add(id);
        }
    }
    return ids;
...
function main() {
    ...
    const knownIds = new Set([...collectKnownRequirementIdsFromDocs(), ...cards.map((c) => c.id).filter(Boolean)]);

    const backendIndex = scope === 'harness' ? { apis: [], tests: [], entities: [] } : safeRead(backendIndexPath, { apis: [], tests: [], entities: [] });
    const apis = backendIndex.apis ?? [];
    const beTests = (backendIndex.tests ?? []).map((t) => ({ source: 'back-end', ...t }));
    const entities = backendIndex.entities ?? [];

    const harnessSelfTestIndex = safeRead(harnessSelfTestIndexPath, { tests: [] });
    const harnessTests = (harnessSelfTestIndex.tests ?? []).map((t) => ({ source: 'harness', ...t }));

    const frontEndIndex = scope === 'harness' ? { pages: [], routes: [], stories: [], tests: [] } : safeRead(frontEndIndexPath, { pages: [], routes: [], stories: [], tests: [] });
    ...
    const tests = [...beTests, ...harnessTests, ...feTests];
    ...
    for (const api of apis) {
        ...
        findings.push({
            ...
            remediation: 'app/docs/standards/api-contract.md'
        });
    }
    ...
    for (const test of tests) {
        ...
        findings.push({
            ...
            remediation: 'harness/docs/standards/acceptance-test.md'
        });
    }
    ...
    for (const entity of entities) {
        ...
        findings.push({
            ...
            remediation: 'app/docs/standards/persistence-schema.md'
        });
    }
    ...
    for (const surface of feSurfaces) {
        ...
        findings.push({
            ...
            remediation: 'app/docs/standards/front-end-project-structure.md'
        });
    }
    ...
    for (const card of cards) {
        for (const ref of card.referencedRequirementIds ?? []) {
            ...
            findings.push({
                ...
                remediation: 'harness/docs/standards/requirement-card.md'
            });
        }
    }
