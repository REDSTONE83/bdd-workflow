...
- `(UI)` 또는 `(E2E)` AC가 있으면 다른 카드(예: REQ-011)의 placeholder 정리, `routes.tsx` 교체, FE BDD 테스트, 시각 스냅샷 기준선이 구현 단계 작업 목록으로 분리되어 있는가
...
- 다른 카드의 placeholder 수용 기준/시나리오/FE BDD 테스트/경로 화면 모의 story 정리. 다른 카드(예: REQ-011)의 placeholder 정리도 구현 단계에서 함께 처리한다.
...
- FE: 새 화면 컴포넌트는 별도 파일로 두고 상호작용 모의 구현까지 작성한다. 외부 API 호출과 navigate 이동은 props/콜백으로 받아 호출 자체는 구현 단계에서 결합한다. `routes.tsx` 교체, 다른 카드 placeholder 정리, `@Covers` FE BDD 테스트, 시각 스냅샷 기준선은 구현 단계로 미룬다.
...
`테스트 승인`과 `구현중` 상태에서는 테스트 누락, 미실행, 스킵, Covers 불일치가 통합 게이트 차단 사유다. 구현 전 실행된 테스트 실패는 RED로 남아도 허용된다. 구현이 끝나면 `검증중` 상태로 올리고 scope에 맞게 `npm run app:validate` 또는 `npm run harness:validate`로 모든 RED를 해소한다. Skeleton 단계의 RED는 정상이며, `@Covers` FE BDD 테스트, 시각 스냅샷 기준선, `routes.tsx` 교체, 다른 카드 placeholder 정리는 Skeleton 승인 이후 단계에서 진행한다.
