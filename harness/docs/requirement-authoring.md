...
- 화면/라우팅 Skeleton: 화면 이름, 업무 진입점, 예상 경로 초안, 접근 권한, 주요 표시 정보. UI가 있는 요건은 Storybook 모의 story로 사용자가 관찰할 상태를 검토할 수 있게 한다.
- 실행 테스트: 승인된 검증 설계를 JUnit Acceptance Test, Storybook Vitest story test, 또는 live Playwright smoke로 옮긴 코드. 요건 Skeleton 승인 후, 실제 구현 전에 작성하고 `테스트 승인`을 거친다.
...
`범위`에만 적혀 있고 `수용 기준`에 없는 화면 구성은 Storybook Vitest로 커버되지 않으므로 사용자가 보는 표시가 의도와 달라져도 RED로 잡히지 않는다. 화면이 범위에 있는 카드는 Skeleton 승인을 요청하기 전에 카드 구성 AC가 한 줄 이상 있는지 다시 확인한다.
...
- `(UI)` 또는 `(E2E)` AC가 있으면 Storybook으로 검토할 Skeleton 상태와 테스트 작성 단계의 Storybook Vitest/live Playwright 커버가 구분되어 있는가
...
- FE 공통 기본 UI 컴포넌트나 주요 화면 조각을 새로 만들거나 바꾸는 요건이면 본 카드 Skeleton에서 함께 작성한 Storybook story 위치가 명시되어 있는가
...
백엔드는 인터페이스와 계약까지만 만든다. 프런트엔드 화면은 사용자가 Storybook에서 사용자 흐름을 검토할 수 있도록 상호작용 모의 구현 수준까지 작성한다. 단, 외부 API client 결합과 `routes.tsx` 교체, 다른 카드 placeholder 정리는 구현 단계로 분리해 Skeleton 검토가 placeholder 변동에 흔들리지 않도록 한다.
...
- `previewSchema` 산출물
- 화면/라우팅 Skeleton: 화면 이름, 업무 진입점, 예상 경로 초안, 접근 권한, 주요 표시 정보, 사용자가 관찰할 상태(초기(initial) / 입력 항목 오류(fieldErrors) / 제출 중(submitting) / 서버 거절(serverRejection) / 성공(success) 등). 카드의 `BDD 테스트 리뷰 > 요건 Skeleton 승인 이력`에 작성 위치와 함께 남긴다.
...
- Storybook story: 화면이 있는 카드는 경로 화면 모의 story 1개와 사용자가 관찰할 상태별 story 묶음(초기(initial) / 입력 항목 오류(fieldErrors) / 제출 중(submitting) / 서버 거절(serverRejection) / 성공(success) 등)을 Skeleton 승인 전에 작성한다. story 파일에 `parameters.harness.requirements`로 본 카드를 연결하고, `parameters.docs.description.component` 또는 `parameters.docs.description.story`에 화면 목적, 주요 요소, 사용자 흐름, 관찰 포인트를 Markdown 설명으로 남긴다. 설명 문장은 한국어 업무 용어를 우선 사용하고, 코드/API 용어가 필요한 경우에만 백틱으로 남긴다. Storybook Docs 설명에는 범위 섹션을 두지 않는다. 검토 범위는 요건 카드의 `UI Skeleton`과 `Storybook 계약`에서 관리한다.

금지:

- `@Covers`가 붙은 테스트 메서드 또는 Storybook Vitest/live Playwright `Covers` 메타데이터. Skeleton 승인 요청 시 JUnit `@Covers`, Storybook Vitest `covers`, live Playwright `Covers`가 존재하면 승인 불가다.
...
FE 구현에서 경로/화면을 추가하거나 변경하면 Skeleton 단계에서 경로 화면 모의 story를 작성하거나 갱신한다. 경로 화면 모의 story는 실제 화면 컴포넌트를 `MemoryRouter`, 모의 인증 provider, 모의 API provider처럼 필요한 최소 provider로 감싸 경로 진입 화면을 Storybook에서 확인할 수 있게 만든다. 공통 기본 UI 컴포넌트(`app/front-end/src/components/ui`) 또는 주요 화면 조각을 추가하거나 변경할 때도 Storybook story를 작성하거나 갱신한다. Skeleton story에는 `parameters.harness.requirements`로 관련 요건을 연결하고 기본(normal) / 비활성화(disabled) / 로딩(loading) / 오류(error) / 열림(open) / 비어 있음(empty) 같은 사용자가 관찰할 상태를 고정한다. 테스트 작성 단계에서는 AC를 검증하는 story에 `tags: ["test"]`, `parameters.harness.covers`, 사용자 행위와 `expect(...)`가 포함된 `play` 함수를 함께 추가해 완료 판정 입력으로 만든다. Docs 개요는 대표 story와 설명에 집중하고, 하위 상태 story를 모두 캔버스로 펼쳐 렌더링하지 않는다. 상태 설명은 Docs 개요의 텍스트 목록에서 확인하고, 상태별 화면은 좌측 story 목록의 이름 붙은 story로 직접 진입해 확인한다. 열린 대화상자처럼 대표 캔버스가 Docs 본문을 덮는 story는 `parameters.harness.docs.omitPrimaryCanvas = true`와 `parameters.harness.docs.omitComponentProperties = true`로 Docs 개요의 대표 캔버스와 컴포넌트 속성을 생략하고, 실제 화면은 이름 붙은 story에서 확인한다. Docs 설명에는 화면 목적, 주요 표시 요소, 사용자가 클릭할 순서, 조작 뒤 관찰할 상태 변화를 포함하되 범위 섹션은 쓰지 않는다. 설명 문장은 한국어 업무 용어를 우선 사용하고, 코드/API 용어가 필요한 경우에만 백틱으로 남긴다. Storybook Docs 전용 스타일은 Markdown 순서 목록과 글머리표를 보존해야 한다. Skeleton 승인 전에는 story 목록뿐 아니라 실제 `*.stories.tsx` 구현과 `build-storybook` 통과까지 확인한다.
...
초안 단계에서는 `BDD 테스트 리뷰`의 최신 `결과:` 라인을 `미완료`로 둔다. 승인 게이트는 `BDD 테스트 리뷰` 섹션의 자유 텍스트가 아니라 최신 `결과:` 라인을 기준으로 `승인`/`미완료`를 판정하며, `Skeleton 결과:`는 테스트 리뷰 결과로 보지 않는다. 시나리오 문서 승인과 Acceptance Test 작성·리뷰가 끝나기 전까지 요건 카드 상태는 `초안` 또는 `검토중`으로 유지한다.
