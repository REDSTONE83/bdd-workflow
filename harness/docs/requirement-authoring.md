...
요건 카드는 단순 요청 기록이 아니라, 구현 전에 모호성을 줄이고 의사결정을 남기는 기준 문서다. 사용자 요청은 Change Set으로 정리하고, 요건 카드는 제품 또는 하네스 명세의 원천으로 유지한다. 이 하네스에서는 요건 카드의 `수용 기준`이 Acceptance Test의 `@Covers`, Storybook Vitest story의 `covers`, 또는 live Playwright 테스트의 `Covers` 메타데이터와 연결되어 구현 완료 여부를 판단한다.
...
-> 상태: 초안 (요건/AC/수용 시나리오 반복 구체화)
-> 상태: 설계 검토중 (API/DB/UI 설계와 UI 설계 검토 표면 작성)
-> 상태: 설계 승인 (Storybook 포함 설계 build/정적 검증)
-> 상태: 테스트 작성중 (실행 테스트를 먼저 작성)
-> 상태: 테스트 승인 (AC별 테스트 연결 확인, 구현 전 실패 허용)
-> 상태: 구현중 (설계 수준을 넘어 실제 구현)
-> 상태: 검증중 (scope validate로 RED 해소)
-> 상태: 승인 (사람이 최종 승인)
```

새 요청이 들어와도 바로 새 REQ를 만들지 않는다. 먼저 기존 원자 요건의 범위/AC를 바꿀지, 여러 REQ를 함께 수정할 Change Set인지, 통합/비기능/정책/구현 슬라이스 카드가 필요한지 판단한다. 기능 변경이나 정책 전환이면 별도 전환 REQ를 만들지 않고 canonical REQ의 최종 범위와 AC를 갱신한다. 변경 전 상태와 작업 순서는 Change Set에 두고, 정책 선택의 근거만 `의사결정 로그`에 남긴다. 수용 기준이 확정되면 시나리오 하나씩 승인받지 않는다. 영향 REQ 집합 단위로 검증 설계와 API/DB/UI 설계를 세울 수 있지만, 명세와 완료 판정은 각 REQ의 AC 단위로 유지한다. 설계 단계는 인터페이스와 계약을 검토하는 단계이므로 업무 로직, 성공 응답 본문, Repository 쿼리 계약, 실행 테스트는 작성하지 않는다. 다만 UI가 있는 요건은 설계 승인 전에 Storybook으로 화면/컴포넌트/대화상자/목록 상태를 실제 검토할 수 있어야 한다. 동작 내용은 Service/Controller 내부 코멘트와 Storybook 모의 상호작용으로만 설계해 둔다.

## 진행 용어

- 검증 설계: 수용 기준과 `.feature` 수용 시나리오로 무엇을 검증할지 정한 것. API/DB/UI 설계보다 먼저 작성한다.
...
- API/DB 설계: API/DB/Service 계약 골격. 업무 로직은 없고 동작 설계는 내부 코멘트로만 둔다.
- UI 설계: 화면 이름, 업무 진입점, 예상 경로 초안, 접근 권한, 주요 표시 정보. UI가 있는 요건은 Storybook 모의 story로 사용자가 관찰할 상태를 검토할 수 있게 한다.
- 실행 테스트: 승인된 검증 설계를 JUnit Acceptance Test, Storybook Vitest story test, 또는 live Playwright smoke로 옮긴 코드. 설계 승인 후, 실제 구현 전에 작성하고 `테스트 승인`을 거친다.
- 업무 구현: Service 업무 로직, Controller 성공 응답 본문, 실제 FE 컴포넌트/라우팅/API client 연결을 작성하는 단계다.
- 검증: `npm run app:validate` 또는 `npm run harness:validate`로 scope별 AC, 수용 시나리오, 테스트, API 설계, DB 설계, UI 설계 연결과 테스트 결과를 확인하는 단계다. FE 로컬 검증은 `cd app/front-end`에서 별도로 실행한다.
...
범위 문장을 바로 수용 기준으로 옮기지 않는다. 먼저 Acceptance Test, Storybook Vitest, 또는 live Playwright 테스트의 어셔션을 만들 수 있는지 확인한다. 아래 항목 중 하나라도 답이 없으면 사용자에게 확인 질문을 하고 `열린 질문`에 둔다. 답변이 오면 정책 선택은 `의사결정 로그`에, 검증 가능한 결과는 `수용 기준`에 반영한 뒤 `열린 질문`에서 제거한다.
...
- 화면 상태: 로딩(loading), 비어 있음(empty), 오류(error), 비활성화(disabled), 제출 중(submitting), 성공(success), 권한 없음(permission denied), 찾을 수 없음(not found) 중 어떤 상태가 사용자가 관찰해야 하는 결과인가?
- 화면 레이아웃과 카드 구성: 화면이 어떤 레이아웃 단위(전역 헤더 + 사이드바가 있는 보호 화면, 비인증 중앙 단일 카드 등)에 속하고, 그 카드 또는 주요 컨테이너는 어떤 표시 요소(제목, 입력, 버튼, 링크, 안내 영역 등)로 구성되는가? 화면 간 공통 입력 UX(자동 포커스, autocomplete 속성, 비밀번호 보이기/가리기 토글 등)는 본 카드 AC로 둘지, FE 공통 표준에 위임할지 정했는가?
...
제품 기능이 API와 화면을 모두 가지면 별도 "화면 요건"을 먼저 만들지 않는다. 기본 구조는 하나의 원자 기능 카드가 `(API)` 계약 AC와 `(UI)` 화면 AC를 함께 소유하는 것이다. 사용자가 보는 생성, 목록, 수정, 삭제, 상태 변경 결과는 해당 업무 동사의 완료 조건이므로 API 계약과 같은 카드에서 추적한다.

예외는 화면 자체가 독립 제품 능력일 때만 허용한다. 예를 들어 공통 보호 앱 셸, 전역 내비, 인증 gate, 여러 하위 기능을 단순 조합하는 route shell처럼 특정 업무 동사 하나로 귀속되지 않는 경우다. 구현 순서 때문에 임시 화면 슬라이스 카드가 생겼다면 최종 구조에서는 그 카드의 AC, `.feature`, Storybook Vitest `requirements`/`covers`, live Playwright `Requirement`/`Covers` metadata를 canonical 원자 기능 카드로 옮긴다. 화면 슬라이스 카드를 남길 때는 CRUD 같은 업무 AC를 반복하지 않고 route/shell/layout 같은 조합 책임만 둔다.

상위 요건은 하위 원자 요건의 API/UI AC를 복사하지 않는다. 상위 카드가 남아야 한다면 실제 사용자 전체 여정 하나를 `(E2E)` AC로 두고, 세부 생성/목록/수정/삭제/상태 변경 AC는 하위 원자 요건이 소유한다.
...
1. `질문해야 하는 내용`의 화면 레이아웃·카드 구성 질문을 선택지형으로 사용자에게 던진다. 기본 레이아웃 후보(보호 화면 공통 틀 사용 / 비인증 중앙 단일 카드 / 화면 가운데 대화상자 / 화면 전체 사용 등)와 카드의 주요 표시 요소(제목, 입력, 버튼, 링크, 안내 영역 등) 후보를 함께 제시하고, 같은 도메인의 기존 화면 카드(예: REQ-011 로그인)와 같은 패턴을 권장안으로 둔다.
...
4. 입력 UX 세부(자동 포커스, autocomplete 속성, 비밀번호 보이기/가리기 토글, 키보드 제출 등)는 본 카드 AC로 둘지 FE 공통 표준([`../standards/front-end-ui.md`](../standards/front-end-ui.md))에 위임할지를 별도 선택지형 질문으로 확정하고, 결정과 근거를 `의사결정 로그`에 남긴다. 본 카드 AC에 두기로 한 항목만 별도 수용 기준으로 적고, 위임한 항목은 카드 AC에 복제하지 않는다.

`범위`에만 적혀 있고 `수용 기준`에 없는 화면 구성은 Storybook Vitest로 커버되지 않으므로 사용자가 보는 표시가 의도와 달라져도 RED로 잡히지 않는다. 화면이 범위에 있는 카드는 설계 승인을 요청하기 전에 카드 구성 AC가 한 줄 이상 있는지 다시 확인한다.

화면 구성과 동작을 적을 때 UI 컴포넌트는 UI 컴포넌트 어휘 표준(`harness/docs/standards/ui-vocabulary.md`)의 정규 명칭을 쓴다. `다이얼로그`/`모달`/`팝업`은 `대화상자`(확인·입력 대화상자), `인풋`은 `텍스트 입력`, `체크 박스`는 `체크박스`로 적는다. 이 별칭은 terminology ban으로 본문에서 `BAN_VIOLATION` 차단되고, `ui.button`·`ui.dialog` 같은 컴포넌트 원자 키는 카드 `## 표준 용어`에 등록하지 않는다(`CARD-TERM-UI-PRIMITIVE`).

### 화면 카드 AC 점검

화면을 포함하는 카드는 위 화면 레이아웃 AC 외에 다음을 설계 승인 전에 한 줄씩 확인한다. 정적 룰이 아니라 작성자·리뷰어의 공통 사고 틀이며, 실제 리뷰에서 반복적으로 누락된 항목을 모았다.
...
- **범위 ↔ AC 행위 대칭**: `범위`에 적은 모든 사용자 관찰 동작이 검증 가능한 AC로도 있는가? 화면 구성뿐 아니라 보호 라우트 가드(비인증 → 로그인 이동), 로그인 후 복귀, 성공 후 이동, 모드별 동작도 포함한다. `범위`에만 있고 `수용 기준`에 없는 동작은 Storybook Vitest로 커버되지 않아 RED로 잡히지 않는다.
...
수용 기준은 사용자/PO/QA가 함께 승인하는 **완료 기준**이다. 수용 시나리오만 관계자 언어로 정리해도 AC가 기술 어휘로 가득하면 원천 기준이 다시 개발자 문서가 된다. AC도 같은 방향을 따른다.
...
AC가 확정되면 요건 하나를 기준으로 검증 설계와 API/DB/UI 설계를 만든다. 검증 설계는 수용 기준과 업무 시나리오가 맞는지 확인하는 기준이고, API/DB/UI 설계는 API 계약, DB 스키마, Service 공개 메서드, 화면 표면이 서로 맞는지 검토하기 위한 골격이다. 시나리오 하나씩 승인받지 않고, 요건 단위로 설계를 검증하고 사용자 승인을 받은 뒤 같은 요건의 구현으로 넘어간다.
...
Cucumber 실행 도구는 도입하지 않는다. `.feature`는 "공유 수용 시나리오 + 하네스 추적 입력"으로만 사용한다. 테스트 실행은 평소처럼 JUnit이 담당하며, `.feature`는 사람이 읽고 하네스가 파싱한다.
...
API/DB/UI 설계는 별도 사람이 관리하는 계약 섹션이 아니라 코드 골격(Controller/DTO/Entity/Repository/Service), 화면/라우팅 설계, `previewSchema`와 source index 산출물로 확인한다. `.feature` 파일은 시나리오 본문에만 집중하고, API/DB/Service 명세는 평소대로 `app/back-end/src/main/java`의 골격 코드에서 확인한다. `(UI)` 또는 `(E2E)` AC가 있는 요건은 화면 이름, 업무 진입점, route 초안, 접근 권한, 주요 표시 정보, 필요한 Storybook 상태를 카드의 설계 승인 이력에 남긴다. 작성 기준은 `app/docs/standards`의 API/DB/FE 표준을 따른다.
...
- 이 행위가 정상 UI 흐름인가? 정상 흐름이 아니라면 북마크, 딥링크, 브라우저 뒤로가기/새로고침, 다중 기기에서 데이터가 줄어든 상태, 외부 통합 URL처럼 사용자가 실제로 마주칠 수 있는 경로를 `Given`에 명시했는가? 그런 경로를 설명할 수 없는 행위는 수용 시나리오로 두지 말고 API 방어 계약으로 분류한다. ([`../standards/acceptance-test.md`](../standards/acceptance-test.md) "발생 경로가 설명되지 않는 행위는 시나리오로 두지 않는다")
...
- 실행 환경 경계가 있다면 코드 설계, 설정 설계, 테스트 전략에서 그 차이를 재현할 수 있는가?
- 생성, 발급, 저장, 시작되는 상태가 있다면 삭제, 만료, 해제, 정리의 scope가 같은가?
- 외부 계약이 있다면 카드 AC와 설계 코드 양쪽에 공개/보호 범위와 호환성 정책이 반영됐는가?
...
- Service 공개 메서드가 설계 단계에서도 `transaction.md`의 메서드 레벨 `@Transactional` 정책을 따르는가?
- Service 내부 코멘트에 정상 흐름, 예외 흐름, 상태 변화, 알림/부수효과, 트랜잭션 경계 설계가 요약되어 있는가
- 설정 설계가 있다면 프로젝트 소유 `app.*` 키가 typed `@ConfigurationProperties`에 바인딩되고, profile override 정책이 명확한가?
- `(UI)` 또는 `(E2E)` AC가 있으면 화면 이름, 경로 초안, 접근 권한, 주요 표시 정보, 사용자가 관찰할 상태(초기(initial) / 입력 항목 오류(fieldErrors) / 제출 중(submitting) / 서버 거절(serverRejection) / 성공(success) 등)가 요약되어 있는가
...
- `(UI)` 또는 `(E2E)` AC가 있으면 새 화면 컴포넌트가 `routes.tsx` 교체 없이 별도 파일로 작성되어 있고 상호작용 모의 구현(폼 입력 반응, 브라우저 쪽 검증 안내, 상태 전환)이 실제로 동작하는가
- `(UI)` 또는 `(E2E)` AC가 있으면 외부 API 호출과 navigate 이동이 컴포넌트 props/콜백으로 mock되어 있고, 화면이 직접 `apiClient.*`을 호출하지 않는가 (실제 결합은 구현 단계)
- `(UI)` 또는 `(E2E)` AC가 있으면 경로 화면 모의 story 1개와 사용자가 관찰할 주요 상태별 story 묶음(초기(initial) / 입력 항목 오류(fieldErrors) / 제출 중(submitting) / 서버 거절(serverRejection) / 성공(success) 등)이 작성되어 있고, `parameters.harness.requirements`로 본 카드에 연결되어 있는가
- `(UI)` 또는 `(E2E)` AC가 있으면 Storybook으로 검토할 설계 상태와 테스트 작성 단계의 Storybook Vitest/live Playwright 커버가 구분되어 있는가
- `(UI)` 또는 `(E2E)` AC가 있으면 다른 카드(예: REQ-011)의 placeholder 정리, `routes.tsx` 교체, Storybook Vitest/live Playwright 커버 테스트, 시각 스냅샷 기준선이 구현 단계 작업 목록으로 분리되어 있는가
- FE 공통 기본 UI 컴포넌트나 주요 화면 조각을 새로 만들거나 바꾸는 요건이면 본 카드 설계에서 함께 작성한 Storybook story 위치가 명시되어 있는가
- `@Covers`가 붙은 JUnit Acceptance Test나 Storybook Vitest/live Playwright `Covers` 메타데이터가 아직 저장소의 추적 대상 경로에 생성되지 않았는가?
- 설계 코드에 실제 업무 로직이나 임시 성공 응답이 들어가지 않았는가

### 설계 단계 산출물 범위

설계 승인 전까지는 "인터페이스와 계약"까지만 만든다.

백엔드는 인터페이스와 계약까지만 만든다. 프런트엔드 화면은 사용자가 Storybook에서 사용자 흐름을 검토할 수 있도록 상호작용 모의 구현 수준까지 작성한다. 단, 외부 API client 결합과 `routes.tsx` 교체, 다른 카드 placeholder 정리는 구현 단계로 분리해 설계 검토가 placeholder 변동에 흔들리지 않도록 한다.
...
- `previewSchema` 산출물
- 화면/라우팅 설계: 화면 이름, 업무 진입점, 예상 경로 초안, 접근 권한, 주요 표시 정보, 사용자가 관찰할 상태(초기(initial) / 입력 항목 오류(fieldErrors) / 제출 중(submitting) / 서버 거절(serverRejection) / 성공(success) 등). 카드의 `수용 테스트 리뷰 > 요건 설계 승인 이력`에 작성 위치와 함께 남긴다.
- FE 화면 상호작용 모의 구현: 새 화면 컴포넌트를 별도 파일로 작성한다. 실제 DOM과 Tailwind 스타일을 사용해 화면이 실제처럼 보이게 하고, 폼 입력 반응, 브라우저 쪽 검증 안내, 제출 중/오류/성공 등 사용자가 관찰할 상태 전환이 실제로 동작하게 한다. 외부 API 호출과 라우팅 이동은 컴포넌트의 props/콜백(또는 React Router 의존성 주입)으로 받아 Storybook control이 결과를 강제할 수 있게 한다.
- Storybook story: 화면이 있는 카드는 경로 화면 모의 story 1개와 사용자가 관찰할 상태별 story 묶음(초기(initial) / 입력 항목 오류(fieldErrors) / 제출 중(submitting) / 서버 거절(serverRejection) / 성공(success) 등)을 설계 승인 전에 작성한다. story 파일에 `parameters.harness.requirements`로 본 카드를 연결하고, `parameters.docs.description.component` 또는 `parameters.docs.description.story`에 화면 목적, 주요 요소, 사용자 흐름, 관찰 포인트를 Markdown 설명으로 남긴다. 설명 문장은 한국어 업무 용어를 우선 사용하고, 코드/API 용어가 필요한 경우에만 백틱으로 남긴다. Storybook Docs 설명에는 범위 섹션을 두지 않는다. 검토 범위는 소스 기반 `UI 설계`와 카드의 `UI 설계 검토 표면`에서 관리한다.

금지:

- `@Covers`가 붙은 테스트 메서드 또는 Storybook Vitest/live Playwright `Covers` 메타데이터. 설계 승인 요청 시 JUnit `@Covers`, Storybook Vitest `covers`, live Playwright `Covers`가 존재하면 승인 불가다.
...
- 다른 카드의 placeholder 수용 기준/시나리오/Storybook Vitest 커버 테스트/경로 화면 모의 story 정리. 다른 카드(예: REQ-011)의 placeholder 정리도 구현 단계에서 함께 처리한다.
- `Covers`가 붙은 Storybook Vitest 또는 live Playwright 테스트
- 시각 스냅샷 기준선
...
- Service: 인터페이스나 클래스 시그니처만 작성한다. 클래스 본문이 필요하면 내부 코멘트로 흐름을 적고 `throw new UnsupportedOperationException(...)`으로 닫는다. 이때 공개 서비스 메서드는 설계 단계라도 `@Transactional` 또는 `@Transactional(readOnly = true)`를 메서드 레벨에 명시한다.
- Entity: `previewSchema`를 돌리기 위해 필요한 만큼 완전한 형태로 작성한다.
- FE: 새 화면 컴포넌트는 별도 파일로 두고 상호작용 모의 구현까지 작성한다. 외부 API 호출과 navigate 이동은 props/콜백으로 받아 호출 자체는 구현 단계에서 결합한다. `routes.tsx` 교체, 다른 카드 placeholder 정리, Storybook Vitest/live Playwright `Covers`, 시각 스냅샷 기준선은 구현 단계로 미룬다.

### 승인 게이트

검증 설계 + API/DB/UI 설계를 묶어 먼저 사용자에게 승인을 요청한다. 승인되면 `설계 승인` 상태로 두고, 같은 요건의 실행 테스트를 작성한다. 실행 테스트가 모든 AC와 연결되면 `테스트 승인` 상태로 올린 뒤 실제 구현을 시작한다.

승인 이력은 요건 카드의 `수용 테스트 리뷰 > 요건 설계 승인 이력` 서브섹션에 요건 단위로 남긴다. `.feature` 파일 자체에는 승인 이력을 넣지 않는다. 표준 Gherkin 도구와의 호환을 유지하기 위해서다.

### 설계 검증 명령

설계 단계의 카드는 아직 `@Covers` 테스트가 없으므로 RED로 잡힌다. 이는 정상이다. 이 시점에는 strict 게이트인 `app:validate`/`harness:validate`를 돌리지 않는다. 다음 명령으로 컴파일, 소스 인덱스, 스키마, 추적 현황만 확인한다.

```bash
cd app/back-end
./gradlew compileJava                                # 설계 코드 컴파일 확인
...
npm run build-storybook    # 상호작용 모의 구현 컴포넌트와 경로 화면 모의 story / 상태별 story가 정상 빌드되는지 확인
```

리뷰어가 Storybook에서 사용자 흐름을 직접 확인하려면 `npm run storybook` 으로 띄우고 본 카드의 `parameters.harness.requirements` 가 본 요건 ID로 연결된 story 묶음(경로 화면 모의 story + 상태별)을 본다.

`테스트 승인`과 `구현중` 상태에서는 테스트 누락, 미실행, 스킵, Covers 불일치가 통합 게이트 차단 사유다. 구현 전 실행된 테스트 실패는 RED로 남아도 허용된다. 구현이 끝나면 `검증중` 상태로 올리고 scope에 맞게 `npm run app:validate` 또는 `npm run harness:validate`로 모든 RED를 해소한다. 설계 단계의 RED는 정상이며, `@Covers` Acceptance Test, Storybook Vitest `covers`, live Playwright `Covers`, 시각 스냅샷 기준선, `routes.tsx` 교체, 다른 카드 placeholder 정리는 설계 승인 이후 단계에서 진행한다.

## 수용 테스트 코드 작성

시나리오는 사용자 행위 단위이고 테스트는 AC 검증 단위다. 한 시나리오에 여러 테스트가 귀속될 수 있고, 같은 AC를 입력 변형/경계값마다 별도 테스트로 나누는 것이 권장된다.

승인된 검증 설계의 `.feature` 시나리오 `Covers:`에 포함된 AC를 검증하는 테스트를 작성한다. 백엔드는 JUnit Acceptance Test, 프런트엔드 UI는 Storybook Vitest story test, 상위 통합 성과는 live Playwright smoke가 담당한다. Vitest/Testing Library 단위 테스트는 TDD/보조 테스트로 두며 AC 커버리지에는 포함하지 않는다.
...
FE 테스트 계층은 [`front-end-testing.md`](../../app/docs/standards/front-end-testing.md)를 따른다. Storybook Vitest story는 `parameters.harness.requirements`와 `parameters.harness.covers` literal 메타데이터를 남긴다. live Playwright smoke는 `test.info().annotations.push(...)`에 `Requirement`/`Covers` literal 메타데이터를 남긴다. 하네스는 `npm run app:front-end-source-index`로 이 값을 읽고, Storybook Vitest JUnit 결과(`app/front-end/test-results/storybook-junit.xml`)와 live smoke JSON 결과(`app/front-end/test-results/e2e-live-results.json`)를 병합해 RED/GREEN/BLUE를 판정한다.

Storybook Vitest story test를 작성할 때는 `covers`와 함께 성공 조건을 반드시 남긴다. `play` 함수 또는 `play`가 가리키는 `assert...` 함수 안에 사용자 행위와 `expect(...)` 검증을 두고, 공통 helper는 대화상자 찾기·닫힘 대기 같은 저수준 동작에만 사용한다. 성공 조건은 사용자가 보는 화면 결과로 적는다. 성공 제출은 `userEvent`로 버튼을 누르고 닫힘/목록 반영/경로 변경을 확인하며, DOM API를 직접 호출해 정상 UI 경로를 우회하지 않는다. 실패나 제출 중 상태는 같은 대화상자 또는 양식 안에서 재시도 가능한 안내와 비활성 버튼 상태를 확인한다.

상위 요건의 `(E2E)` AC는 mock API가 아니라 `tests/e2e/live/**/*.live.spec.ts`의 live Playwright smoke로 커버한다. live smoke는 실 Spring Boot 백엔드와 Vite dev server를 함께 띄우고, FE origin proxy와 브라우저 Cookie 인증 흐름까지 포함해 기능 전체 성과를 확인한다. 세부 화면 상태, 입력 검증, 실패 분기는 하위 원자 요건의 Storybook Vitest 테스트로 내려보낸다.

FE 구현에서 경로/화면을 추가하거나 변경하면 설계 단계에서 경로 화면 모의 story를 작성하거나 갱신한다. 경로 화면 모의 story는 실제 화면 컴포넌트를 `MemoryRouter`, 모의 인증 provider, 모의 API provider처럼 필요한 최소 provider로 감싸 경로 진입 화면을 Storybook에서 확인할 수 있게 만든다. 공통 기본 UI 컴포넌트(`app/front-end/src/components/ui`) 또는 주요 화면 조각을 추가하거나 변경할 때도 Storybook story를 작성하거나 갱신한다. 설계 story에는 `parameters.harness.requirements`로 관련 요건을 연결하고 기본(normal) / 비활성화(disabled) / 로딩(loading) / 오류(error) / 열림(open) / 비어 있음(empty) 같은 사용자가 관찰할 상태를 고정한다. 테스트 작성 단계에서는 AC를 검증하는 story에 `tags: ["test"]`, `parameters.harness.covers`, 사용자 행위와 `expect(...)`가 포함된 `play` 함수를 함께 추가해 완료 판정 입력으로 만든다. Docs 개요는 대표 story와 설명에 집중하고, 하위 상태 story를 모두 캔버스로 펼쳐 렌더링하지 않는다. 상태 설명은 Docs 개요의 텍스트 목록에서 확인하고, 상태별 화면은 좌측 story 목록의 이름 붙은 story로 직접 진입해 확인한다. 열린 대화상자처럼 대표 캔버스가 Docs 본문을 덮는 story는 `parameters.harness.docs.omitPrimaryCanvas = true`와 `parameters.harness.docs.omitComponentProperties = true`로 Docs 개요의 대표 캔버스와 컴포넌트 속성을 생략하고, 실제 화면은 이름 붙은 story에서 확인한다. Docs 설명에는 화면 목적, 주요 표시 요소, 사용자가 클릭할 순서, 조작 뒤 관찰할 상태 변화를 포함하되 범위 섹션은 쓰지 않는다. 설명 문장은 한국어 업무 용어를 우선 사용하고, 코드/API 용어가 필요한 경우에만 백틱으로 남긴다. Storybook Docs 전용 스타일은 Markdown 순서 목록과 글머리표를 보존해야 한다. 설계 승인 전에는 story 목록뿐 아니라 실제 `*.stories.tsx` 구현과 `build-storybook` 통과까지 확인한다.

## 테스트 리뷰 체크리스트

- 요건 카드의 모든 수용 기준이 `@Covers`로 연결되어 있는가?
- `@Covers` 문장이 수용 기준과 정확히 일치하는가?
- 모든 수용 테스트의 `@Covers` AC가 같은 요건의 어떤 `.feature` 수용 시나리오 `Covers:`에 포함되어 있는가? (`TEST_COVERS_NO_SCENARIO_COVERS` WARNING 없음)
...
- FE 대상 요건이면 Storybook Vitest story test가 실제 사용자 행위, 주요 화면 표시, 권한/라우팅, 데스크톱 화면 또는 접근성 요구를 필요한 만큼 검증하는가?
- `parameters.harness.covers`가 있는 story마다 `play` 또는 `assert...` 함수에 `expect(...)` 성공 조건이 있는가?
- story test가 `form.requestSubmit(...)` 같은 DOM API 우회 없이 `userEvent`로 사용자가 누르는 버튼과 입력을 조작하는가?
- 대화상자, 메뉴, portal 표면은 현재 열린 role/name 범위 안에서 검증하는가?
- FE TDD 테스트가 class/style 세부값보다 상태별 UI 결과를 검증하는가?
- Storybook story가 공통 컴포넌트와 주요 화면 조각의 상태를 충분히 담는가?

리뷰 결과는 요건 카드의 `수용 테스트 리뷰` 섹션에 요건 단위로 남긴다. 테스트 목록은 수기로 관리하지 않고 하네스 리포트에서 확인한다.
...
- 범위와 제외 범위가 명확하다.
- 수용 기준이 테스트 가능한 문장이다.
- 모든 수용 기준을 커버하는 수용 시나리오 문서가 사용자 승인을 받았다.
- 승인된 시나리오 전체에 대해 수용 테스트 코드 리뷰가 완료되었다.
- scope에 맞는 validate 명령에서 RED가 없다.
- FE 대상 요건이면 `cd app/front-end && npm run validate:full`이 성공한다.

초안 단계에서는 `수용 테스트 리뷰`의 최신 `결과:` 라인을 `미완료`로 둔다. 승인 게이트는 `수용 테스트 리뷰` 섹션의 자유 텍스트가 아니라 최신 `결과:` 라인을 기준으로 `승인`/`미완료`를 판정하며, 레거시 `Skeleton 결과:`는 테스트 리뷰 결과로 보지 않는다. 시나리오 문서 승인과 Acceptance Test 작성·리뷰가 끝나기 전까지 요건 카드 상태는 `초안` 또는 `설계 검토중`으로 유지한다.
