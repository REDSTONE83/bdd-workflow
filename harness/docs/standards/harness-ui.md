...
- feature 이름은 하네스 작업자가 보는 업무 영역으로 정한다. 예: `requirements`, `terminology`, `surfaces`, `tests`, `gates`, `change-sets`, `runs`.
...
- `src/features/surfaces`: API, Entity, UI 표면 목록형 카드 조회.
- `src/features/tests`: 테스트 정의와 수행 결과 목록 조회.
...
- 표면 조회 화면은 선택한 scope의 source index와 trace state를 UI 서버의 `/api/surfaces?scope={harness|application}` DTO로 표시하고, API, Entity, UI 표면의 식별자, 연결 요건, 구현 위치, 요청/응답, 컬럼, Storybook 정보를 화면 DTO에서 보존한다. 화면은 source index 원본 파일을 직접 읽지 않는다.
- 테스트 결과 화면은 선택한 scope의 backend/front-end/self-test source index와 test-results index를 UI 서버의 `/api/tests?scope={harness|application}` DTO로 표시하고, 런타임, 수행 상태, 연결 요건, 구현 위치, freshness 이슈 값을 화면 DTO에서 보존한다. 화면은 결과 파일 위치나 식별자 상세를 표시하지 않고 테스트 성공/실패를 재계산하지 않는다.
...
- 표면 조회: API 탭, Entity 탭, UI 탭, 검색 결과, 빈 scope.
- 테스트 결과: 요약과 목록, 검색/필터 결과, freshness 이슈, 빈 scope.
...
- 1차 내비는 상단 가로 메뉴가 아니라 좌측 LNB로 둔다. LNB 항목은 요건, 표준 용어, API/Entity/UI, 게이트, Change Set, 실행 화면 순서로 고정하고, 본문은 LNB 오른쪽에 표시한다.
- 필터와 상태 요약은 목록 위에 두고, 상세 정보는 선택 후 상세 화면에서 다룬다.
- 표면 조회 화면은 API, Entity, UI 탭으로 구분하고 각 탭은 요건 상세의 UI/API/DB 설계 탭과 같은 세로 목록형 카드로 보여준다. API 카드에는 method, path, operationId, 연결 요건, 구현 위치, 응답 코드, Request/Response 펼침, Request/Response 필드와 중첩 참조 객체 펼침을 둔다. Entity 카드에는 className, table, listener, 연결 요건, 구현 위치, 컬럼 정보를 둔다. UI 카드에는 Page/Route/Story 종류, route 또는 Storybook 식별자, 연결 요건, 구현 위치, Storybook 검토 링크, play/assertion 여부를 둔다.
