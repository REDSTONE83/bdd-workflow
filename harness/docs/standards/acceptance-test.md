...
수용 시나리오는 `app/docs/scenarios/` 또는 `harness/docs/scenarios/`의 Gherkin `.feature` 문서로 작성하고 리뷰한다. Acceptance Test는 승인된 시나리오가 다루는 AC를 실행 가능한 검증 코드로 옮긴다. Cucumber 실행 도구는 도입하지 않으며, `.feature`는 "공유 수용 시나리오 + 하네스 추적 입력"으로만 사용한다.
...
- `app/docs/scenarios/REQ-XXX-*.feature` 또는 `harness/docs/scenarios/REQ-XXX-*.feature`: PO/QA/기획자/프론트엔드/백엔드가 함께 검토하는 수용 시나리오 원본 (Gherkin).
...
실행 테스트 작성은 사용자가 검증 설계와 요건 설계를 승인한 뒤에 진행한다. 작성 절차는 [`requirement-authoring.md`](../requirement-authoring.md)를 참조한다.

## 요건 연결 정책

Acceptance Test의 `@Requirement`는 기본적으로 `명세 역할: 원자 요건`인 REQ를 가리킨다. 상위 요건은 같은 기능 묶음을 표현하기 위한 명세 관계이며, 하위 원자 요건의 AC를 대신 커버하지 않는다.

- 원자 요건 테스트: 생성/조회/수정/삭제, 검증 실패, 권한, 정렬/페이징처럼 독립 승인 가능한 AC를 검증한다.
- 상위 요건 테스트: 여러 원자 요건이 함께 만족되어야 확인되는 `(E2E)` 또는 기능 전체 성과 AC가 있을 때만 직접 연결한다.
- 애플리케이션 상위 요건의 `(E2E)` AC 완료 판정은 실 백엔드와 실 프런트엔드를 함께 실행하는 live Playwright smoke가 담당한다. 백엔드 API lifecycle Acceptance Test는 보조 검증으로 둘 수 있지만, 상위 요건의 E2E 완료 근거를 대체하지 않는다.
- 공통 Entity, 공통 DTO, 공통 보안 구성처럼 여러 원자 요건이 실제로 공유하는 구현은 `@Requirement({"REQ-015", "REQ-016"})`처럼 필요한 원자 요건을 함께 적는다. 단순 분류 목적으로 상위 요건만 붙이지 않는다.
- 하위 원자 요건으로 큰 카드를 분리하면 기존 테스트의 `@Requirement`와 `@Covers`를 새 원자 요건의 AC 기준으로 옮긴다.
- 구현 슬라이스 카드가 canonical REQ로 병합되면 테스트 연결도 canonical REQ 또는 새 원자 REQ로 옮긴다. `대체됨` 카드에는 새 테스트를 연결하지 않는다.

## 테스트 클래스

테스트 클래스는 사용자 목적 또는 API 행위 단위로 작성한다. 백엔드 HTTP API를 검증하는 Acceptance Test는 백엔드 테스트 전용 합성 애너테이션 `@ApiAcceptanceTest`를 사용한다. `@ApiAcceptanceTest`는 `@SpringBootTest`, `@AutoConfigureMockMvc`를 포함한다.
...
HTTP API를 직접 호출하지 않는 애플리케이션 보조 Acceptance Test는 일반 JUnit 테스트로 작성하되 `@Requirement`와 `@Covers`로 추적한다. 하네스/문서/정적 분석 self-test는 애플리케이션 테스트와 섞지 않고 `harness/self-test/tests/*.test.ts`에 둔다. OpenAPI dump처럼 산출물 생성만 담당하고 AC를 검증하지 않는 테스트에는 `@Covers`를 붙이지 않는다.
...
백엔드 테스트와 하네스 self-test는 다음처럼 분리한다.

- `app/back-end/src/test/java`: 애플리케이션 요건을 검증하는 Acceptance Test.
- `app/back-end/src/test/java/com/example/bddworkflow/testsupport`: `ApiAcceptanceTest`, `ApiRequestSupport`, `TestJwt` 같은 백엔드 애플리케이션 테스트 지원 API.
- `harness/annotations/src/main/java`: `Requirement`, `Covers` 같은 최소 추적 애너테이션.
- `harness/self-test/tests/*.test.ts`: 하네스 요건 자체를 검증하는 Node/TypeScript self-test.
- `harness/self-test/support/harness-test.ts`: 하네스 self-test 공통 helper와 메타데이터 wrapper.
- `app/back-end/src/openApiIndex/java`: OpenAPI 산출물 생성 전용 Spring 테스트 어댑터.

`cd app/back-end && ./gradlew test`는 애플리케이션 테스트만 실행한다. `npm run harness:self-test`는 하네스 요건 TypeScript self-test를 실행하고, Node test runner의 JUnit XML 결과는 루트 `build/harness/test-results/nodeSelfTest/junit.xml` 아래에 남긴다.
...
Acceptance Test는 수용 시나리오의 원본이 아니다. 원본은 `app/docs/scenarios/REQ-XXX-*.feature` 또는 `harness/docs/scenarios/REQ-XXX-*.feature`에 둔다.

- 테스트 클래스의 `@Requirement`는 `.feature` 파일의 `@REQ-XXX` 태그와 같은 요건을 가리킨다. 기본 대상은 상위 요건이 아니라 원자 요건이다.
...
`When` 문장이 정상 UI 흐름에서 발생하지 않는 행위라면, 사용자가 실제로 마주칠 수 있는 경로(북마크, 딥링크, 브라우저 뒤로가기/새로고침, 다중 기기에서 데이터가 줄어든 상태, 외부 통합 URL 등)를 `Given`에 명시한다. 그런 경로를 설명할 수 없는 행위는 수용 시나리오가 아니라 API 방어 계약으로 분류한다.
...
API/DB/Service 명세는 컨트롤러·DTO·Entity·Service 소스, 내부 동작 코멘트, `previewSchema` 산출물의 요건 ID 연결에서 추출한 설계 표면으로 본다. `.feature`는 그 위에서 사람이 검토하는 업무 시나리오 한 겹이다.

## 자동 검증 항목

- `cd app/back-end && ./gradlew test`: 애플리케이션 JUnit 테스트 실행, PASS/FAIL/SKIP 수집.
- `npm run harness:self-test`: 하네스 Node/TypeScript self-test 실행, PASS/FAIL/SKIP 수집.
- `npm run app:source-index`: 백엔드 테스트 클래스의 `@Requirement`와 메서드의 `@Covers`/`@DisplayName`을 source index로 수집.
...
- FE 실행 결과 freshness (app scope): Storybook Vitest(`UI` AC)와 live Playwright(상위 `E2E` AC) 결과는 sidecar manifest fingerprint가 현재 FE BDD source fingerprint와 일치할 때만 AC 커버 결과로 인정된다. 테스트의 `Requirement`/`Covers`/식별자를 바꾼 뒤 재실행하지 않으면 옛 결과는 stale로 빠지고 해당 AC는 `NOT_RUN`/`MISSING`으로 계산되며 `FE-TEST-RESULT-STALE` error가 보고된다. stale 해소는 런타임별 재실행으로 한다 — Storybook Vitest는 `npm run app:e2e`, live Playwright는 `npm run app:e2e:live`. manifest는 루트 runner(`app:e2e`/`app:e2e:live`/`app:validate`)만 생성하므로, 프런트엔드 패키지에서 직접 실행하는 `npm run test:storybook`/`e2e`/`e2e:live`는 결과 파일만 갱신해 manifest와 어긋나므로 stale 해소 명령으로 쓰지 않는다. fingerprint는 line을 제거한 stable 식별자만 쓰므로 본문/렌더 코드 편집이나 위쪽 줄 이동만으로는 stale이 되지 않는다. `app:validate`는 두 결과와 manifest를 항상 새로 만든 뒤 판정하므로 stale이 생기지 않는다. (app 작성자 관점 절차는 [`front-end-testing.md`](../../../app/docs/standards/front-end-testing.md).)
- scope별 validate WARNING (report-only, 마이그레이션 완료 후 ERROR 승격 예정):
  - `TEST_COVERS_NO_SCENARIO_COVERS`: 수용 테스트의 `@Covers` AC가 같은 요건의 어떤 `.feature Scenario Covers:`에도 없음.
