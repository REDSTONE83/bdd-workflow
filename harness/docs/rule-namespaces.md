...
| `FE-*` | 프런트엔드 정적 검사 (구조/API 경계/UI 상태/테스트 작성/실행 결과 freshness) | `harness/tools/validate-front-end-standards.mjs` | `front-end.source-index.json`, `test-results.index.json` |
...
FE-TEST-RESULT-STALE      FE 실행 결과(Storybook Vitest·live Playwright)가 현재 FE BDD source와 다른 실행에서 나옴 — manifest fingerprint 불일치/누락 (app scope, severity=error)
