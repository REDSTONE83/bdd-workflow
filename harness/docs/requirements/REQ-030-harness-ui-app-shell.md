...
- 앱셸은 상단 머리 영역(제품명, scope 전환)과 요건, 게이트, Change Set, 실행 화면으로 이동하는 내비로 구성한다.
...
- (UI) 하네스 UI는 모든 화면에서 요건, 게이트, Change Set, 실행 화면으로 이동하는 내비를 표시한다
...
- 화면 표면: `harness/ui/src/features/shell/AppShell.tsx`가 모든 route를 감싸고 기본 진입은 `/requirements`로 둔다. 라우트는 `/requirements`, `/requirements/:requirementId`, `/gate`, `/change-sets`, `/runs`를 가진다.
- 주요 영역: 상단 머리 영역에는 제품명, 현재 scope, scope 전환, 표시 중인 산출물 생성 시각, 오래된 데이터 경고를 둔다. 1차 내비는 요건, 게이트, Change Set, 실행 화면 순서로 둔다. 본문은 현재 route 화면을 표시한다.
...
- 사용자 행위: scope를 전환하고, 1차 내비로 화면을 이동하며, 산출물 없음 안내에서 권장 검증 명령을 확인한다. 화면은 추적 상태와 게이트 판정을 재계산하지 않는다.
...
- 설계일: 2026-06-10
  검증 설계: `.feature`의 7개 Scenario가 카드 수용 기준 7개를 1:1 `Covers:`로 연결한다. localhost 제한은 `(STATIC)`, 내비와 scope/산출물 상태는 `(UI)`로 검증한다.
