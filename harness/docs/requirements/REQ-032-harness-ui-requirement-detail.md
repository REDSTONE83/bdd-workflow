...
- 카드 원본 문서와 연결 항목의 파일 위치를 로컬 에디터로 여는 연결로 제공한다.
...
- (UI) 수용 기준마다 검증 채널, 판정 상태, 연결된 테스트와 시나리오가 항목 카드 안에 표시된다
...
- (UI) 카드 원본 문서와 연결 항목의 파일 위치는 로컬 에디터로 여는 연결로 제공된다
...
- Harness/Requirements/RequirementDetail: CompleteCoverage, RedReasons, BlueBlocked, LinkedArtifacts, AcceptanceAndScenarios, SkeletonContracts
...
- 설계일: 2026-06-10
  ...
  Storybook 계약: `Harness/Requirements/RequirementDetail`의 `CompleteCoverage`, `RedReasons`, `BlueBlocked`, `LinkedArtifacts`, `AcceptanceAndScenarios`, `SkeletonContracts` 상태가 있어야 한다.
