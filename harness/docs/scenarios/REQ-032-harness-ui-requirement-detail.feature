...
Feature: 하네스 UI 요건 상세 추적
  ...
  Scenario: 주요 정보군은 탭으로 구분된다
    ...
    Given 선택한 요건에 메타데이터, 수용 기준, 수용 시나리오, UI 설계, API 설계, DB 설계, 산출물이 있다
    When 하네스 작업자가 요건 상세 화면을 본다
    Then 개요, AC, 수용 시나리오, UI 설계, API 설계, DB 설계, 산출물/소스 탭이 보인다
  ...
  Scenario: AC 목록과 수용 시나리오 목록을 확인한다
    ...
    Given 선택한 요건에 수용 기준과 수용 시나리오가 있다
    And 수용 시나리오의 Covers가 coverage row와 연결되어 있다
    When 하네스 작업자가 AC 탭과 수용 시나리오 탭을 확인한다
    Then 수용 기준 원문, 검증 채널 뱃지, 판정과 연결 테스트 바로가기가 카드 목록으로 보인다
    And 수용 시나리오 제목, Covers 관계, 커버리지 판정, 연결 테스트, 번호 없는 Given/When/Then, feature 파일 위치가 보인다
  ...
  Scenario: 연결 산출물과 원본 위치는 작업자가 열 수 있는 링크로 제공된다
    ...
    Given 선택한 요건에 요건 카드와 수용 시나리오 산출물이 있다
    And 선택한 요건에 API 설계, Request, Response, DB 설계, UI Page, UI Story 소스코드 위치가 있다
    When 하네스 작업자가 산출물/소스 탭을 확인한다
    Then 연결 산출물은 요건 카드와 수용 시나리오 종류 뱃지만 보인다
  ...
  Scenario: API와 DB 설계 연결을 확인한다
    ...
    And API 설계 탭에서 Request와 Response 이름과 필드 구성은 펼침 영역에서 보인다
    And Response의 중첩 객체 필드는 참조 객체 펼침 영역에서 보인다
    And DB 설계 탭에서 DB table이 카드의 주 정보로 보이고 JPA className, listener와 구현 위치는 보조 정보로 보인다
    And DB 설계 컬럼의 columnName, PK 여부, nullable, unique, updatable, length가 먼저 보이고 fieldName, javaType, annotation, 연결 요건은 보조 정보로 컬럼 목록 펼침 영역에서 보인다

  Scenario: UI 설계는 Storybook 링크로 검토한다
    Covers:
      - 연결된 UI 설계는 UI 설계 탭에서 세로 목록형 카드로 표시되고 카드별 설명과 Storybook 검토 링크와 구현 파일 위치가 제공된다

    Given 선택한 요건에 연결된 UI Storybook story가 있다
    And 연결된 UI 설계에 카드별 설명이 있다
    When 하네스 작업자가 UI 설계 탭을 확인한다
    Then UI 설계가 세로 목록형 카드로 보인다
    And 카드별 설명과 Storybook 검토 링크가 보인다
