...
build/{app|harness}/
  ...
  test-results/                                # scope별 테스트 결과 XML
    ...
    # app scope FE 실행 결과(canonical 위치는 app/front-end/test-results/)는 sidecar manifest를 함께 둔다:
    #   storybook-junit.xml      + storybook-junit.manifest.json     (Storybook Vitest, UI AC)
    #   e2e-live-results.json    + e2e-live-results.manifest.json    (live Playwright, 상위 E2E AC)
...
- `card`: `id`, `title`, `status`, `priority`, `requirementType`, `specRole`, `targetSystem`, `productArea`, `qualityAttributes[]`, `verificationLevel`, `parentRequirementIds[]`, `relatedRequirementIds[]`, `replacedByRequirementIds[]`, `purpose`, `scopeItems[]`, `acceptanceCriteria[]`, `openQuestions[]`, `terms[]`, `outOfScopeItems[]`, `decisionLogs[]`, `sectionPresent`, `approved`, `apiDesign[]`, `dbDesign[]`, `uiDesign[]`, `uiReviewSurfaces[]`, `acceptanceTestReviewResult`, `acceptanceTestReviewIncomplete`, `acceptanceTestReviewApproved`. `purpose`는 `사용자/목적` 섹션의 본문 문자열이다. `scopeItems[]`, `outOfScopeItems[]`, `terms[]`는 각 섹션의 bullet 목록이다. `decisionLogs[]`는 `의사결정 로그` 섹션의 `{ date, decision, reason, decisionMaker, impact }` 목록이다. `parentRequirementIds[]`는 카드 헤더의 `상위 요건`에서 온 단일 소스이며, `없음`이면 빈 배열이다. `acceptanceCriteria[]`는 `string`이 아니라 `{ text, target, invalidMarker?, line }` 객체 배열이다. `line`은 카드 본문 전체 기준 1-based AC bullet 줄 번호다. `text`는 bullet 시작의 마커 토큰을 제거한 원문, `target`은 `API | UI | E2E | STATIC | null`, `invalidMarker`는 bullet 시작에 마커처럼 보이는 토큰이 있으나 허용 목록 밖일 때만 채워진다. 같은 카드의 `@Covers`, Storybook Vitest `covers`, live Playwright `Covers`, `.feature` `Covers:` 매칭은 모두 `text` 값으로 한다. `apiDesign[]`, `dbDesign[]`, `uiDesign[]`, `uiReviewSurfaces[]`는 새 카드에서 수작업 원천이 아니라 전환 기간 호환 파서가 레거시 섹션을 읽어 담을 수 있는 보조 필드다. 생성 설계 표면의 원천은 trace state의 `designSurfaces`다. `acceptanceTestReviewResult`는 `수용 테스트 리뷰` 섹션에서 최신 `결과:` 라인만 추출한 `{ line, status, normalizedStatus } | null`이며, 레거시 `Skeleton 결과:`와 자유 텍스트는 제외한다. `acceptanceTestReviewIncomplete`/`acceptanceTestReviewApproved`는 이 최신 결과 라인에서 계산한다. 전환 기간 동안 `verificationTargets`, `apiSkeleton[]`, `dbSkeleton[]`, `uiSkeleton[]`, `storybookContract[]`, `bddReviewResult`, `bddReviewIncomplete`, `bddReviewApproved` 필드는 같은 값의 legacy alias로 남을 수 있다.
...
- `test-result`: `identity`, `alternateIdentities[]`, `status` (`PASS` | `FAIL` | `SKIP` | `NOT_RUN`), `runtime` (`junit` | `playwright` | `node` | `storybook-vitest`). 엔트리 `kind`는 항상 `"test-result"`이고, runner 구분은 `runtime` 필드를 쓴다. app scope FE 실행 결과(`storybook-vitest`, `playwright`)는 sidecar manifest fingerprint가 현재 FE BDD source fingerprint와 일치할 때만 `test-results.index.json`에 수집된다(아래 freshness 계약).
- FE 실행 결과 freshness manifest (`app/front-end/test-results/{storybook-junit,e2e-live-results}.manifest.json`): 결과 파일과 같은 실행에서 `run.mjs` wrapper가 생성하는 sidecar다. 최상위에 `schemaVersion`, `source` (`fe-test-result-manifest`), `runtime` (`storybook-vitest` | `playwright`), `resultFile` (repo-relative canonical 경로), `resultFileSha256` (결과 파일 내용 SHA-256), `startedAt`, `completedAt`, `exitStatus`, `entries[]`를 둔다. 각 `entries[]`는 `{ resultKeys[], requirements[], covers[], fingerprint }`이며 `fingerprint`는 line을 제거한 stable 식별자(`resultKeys`)·`Requirement`·`Covers`만 dedupe·정렬해 정규 직렬화한 값의 SHA-256이다(`harness/tools/test-result-fingerprint.mjs` 공유 모듈). AC `target`·파일 전체 hash·line 포함 `identity`는 fingerprint에 넣지 않는다. 결과 파일과 manifest는 publish/hydrate에서 한 단위로 함께 교체한다.
- `test-results.index.json`의 `issues[]` (REQ-012, app scope): `index-test-results.mjs`가 manifest fingerprint를 현재 FE source fingerprint와 비교해 stale을 빼고 남기는 진단이다. `{ kind: "FE_TEST_RESULT_STALE", runtime, reason, resultFile, identity?, location? }` 형태이며 `reason`은 `fingerprint-mismatch` | `manifest-missing` | `result-file-hash-mismatch`. Layer 2 `validate-front-end-standards.mjs`가 같은 모양을 `FE-TEST-RESULT-STALE` error finding으로 정규화한다. harness scope는 manifest가 없어 이 필터를 적용하지 않는다.
  ...
  "requirements": [
    {
      ...
      "designSurfaces": {
        "api": [/* Controller/DTO/OpenAPI 기반 API 설계 표면 */],
        "db": [/* Entity/schema 기반 DB 설계 표면 */],
        "ui": [/* page/route/story 기반 UI 설계 표면 */]
      },
      "coverage": [
        {
...
`redReasons[]`는 `{ ruleId, message, evidence }` 객체 배열이다. `ruleId`는 [`rule-namespaces.md`](./rule-namespaces.md)의 `TRACE-*` prefix 중 하나(`TRACE-AC-EMPTY` / `TRACE-AC-MISSING` / `TRACE-AC-FAIL` / `TRACE-AC-NO-FEATURE` / `TRACE-DESIGN-*-MISSING`)다. `message`는 리포트에 사람 친화적으로 노출되는 한 줄 문자열이고, `evidence`는 ruleId별로 의미 있는 보조 데이터(예: AC 문장, status, 대상 카드 ID, 누락 설계 종류)를 담는다. 리포터(`render-trace-report.mjs`)는 `- [{ruleId}] {message}` 형태로 prefix를 노출한다.

하네스 UI 요건 상세 DTO는 `coverage[]`, scenario index의 scenario 항목, `apis[]`, `entities[]`, `frontEnd.pages/routes/stories/apiUsages/apiCalls[]`, `designSurfaces`와 각 source index 원본을 얇게 정리해 AC 목록, 수용 시나리오 목록, API 설계, Request, Response, DB 설계, UI 설계 연결을 표시할 수 있다. 이 DTO는 새 판정을 만들지 않으며, 원천 항목의 AC 문장/channel/status/tests/scenarios, scenario title/covers/steps/file/line, method/path/operationId, parameters/returnType, DTO className/fields, Entity table/className/listeners/columns, story title/story/file/line을 화면에 맞게 묶는다. API 설계 탭의 `dataShapes[]`는 Request/Response와 참조 객체용 DTO shape를 담고, DB 설계 탭의 `entitySurfaces[]`는 trace state `entities[]`의 DB table과 JPA Entity/column 메타데이터를 담는다. DB 설계 화면 표현은 DB table과 columnName, PK 여부, nullable, unique, updatable, length를 우선하고 JPA className, listener, fieldName, javaType, annotation은 보조 정보로 둔다. 요건 상세 화면에서 테스트 정보는 별도 테스트 탭이 아니라 AC 항목과 수용 시나리오 항목에 포함한다. AC 항목은 같은 AC 문장의 `coverage[]` row에서 테스트를 가져오고, 수용 시나리오 항목은 scenario `covers[]`와 일치하는 `coverage[]` row들의 테스트를 중복 제거해 보여준다. 연결 산출물 항목은 `card`, `scenario` 두 종류만 사용한다. 소스코드 항목은 연결 산출물 파일을 제외하고 `api:{operationId}`, `Request:{name}`, `Response:{name}`, `Entity:{table}`, `Page:{name}`, `Route:{name}`, `Story:{name}`처럼 종류와 세부 이름을 분리할 수 있는 값을 권장한다. 화면 뱃지는 `Page`/`Route`/`Story`를 `UI Page`/`UI Route`/`UI Story`로 표시한다. UI 설계는 카드별 `description`을 선택 필드로 가질 수 있으며, Storybook URL은 story title과 named export에서 파생한 검토 링크로 제공할 수 있고, 파일 위치는 구현 확인용 보조 링크로 유지한다. `dataShapes[]`는 반환 DTO만 담으며 상태 코드(201/400 등)는 DTO shape가 아니므로 제외한다. `ResponseEntity<T>`·`JsonNullable<T>` 같은 wrapper 타입은 벗기고, 본문이 없는 `Void` 응답은 shape를 만들지 않는다. 제네릭 envelope(예: `PageResponse<T>`)은 envelope DTO의 필드를 담되 type variable을 실제 타입 인자로 치환해(예: `content: List<TodoResponse>`) 표시하고, 치환된 타입이 참조하는 DTO를 중첩 `Object` shape로 펼친다.
...
| 카테고리 | 입력 | 차단 조건 |
| -------- | ---- | --------- |
| `TRACE`  | `trace.state.json` `requirements[]` (선택 카드 ID로 다시 카운트한 RED/GREEN/total) | `--check`: `total === 0`, 상태 미기재 legacy RED, `설계 승인`/`테스트 작성중`/`테스트 승인`/`구현중`의 설계 표면 누락 RED, `테스트 승인`/`구현중`의 테스트 누락·미실행·스킵 RED, `검증중`/`승인` RED. `--require-blue`: 그 위에 `승인` 상태 GREEN. |
