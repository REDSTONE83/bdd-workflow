import type { Meta, StoryObj } from "@storybook/react-vite";
import { expect, userEvent, within } from "storybook/test";
import { CommandRunner } from "./CommandRunnerPage";
import { commands, failedRun, readyRun, rejectedRun, requirementRows, runningRun, succeededRun } from "../../lib/harness-data/fixtures";
import { LoadingState } from "../../components/ui/LoadingState";
import { ErrorState } from "../../components/ui/ErrorState";

const meta = {
  title: "Harness/Runs/CommandRunner",
  component: CommandRunner,
  parameters: {
    harness: { requirements: ["REQ-035"] },
    docs: {
      description: {
        component: "허용된 검증 명령 선택, 선택된 명령 카드 내부의 단일 요건 검색/선택과 실행 버튼, 실행 로그, 단일 실행 잠금, 서버 거절 상태를 검토한다.",
      },
    },
  },
  tags: ["autodocs", "test"],
} satisfies Meta<typeof CommandRunner>;

export default meta;
type Story = StoryObj<typeof meta>;

export const Ready: Story = {
  args: { commands, run: readyRun, requirements: requirementRows },
  parameters: {
    harness: {
      covers: [
        "실행 화면은 허용된 검증 명령 목록을 표시하고 선택해 실행할 수 있다",
        "단일 요건 추적 명령은 실행 대상 요건을 선택 항목으로 제공하고 초기 미선택 상태와 선택 해제를 지원한다",
      ],
    },
    docs: {
      description: {
        story: "명령 실행 전 준비 상태다. 실행 대상 요건이 미선택으로 시작하고, 선택된 명령 카드 내부에서 요건 검색/선택 진입점과 실행 버튼이 함께 보이는지 확인한다.",
      },
    },
  },
  play: async ({ canvasElement }) => {
    const canvas = within(canvasElement);
    for (const commandId of ["harness:trace", "harness:validate", "harness:self-test", "app:trace", "app:validate", "repo:validate"]) {
      await expect(canvas.getAllByText(commandId)[0]).toBeVisible();
    }
    await expect(canvas.queryByText("app:e2e")).not.toBeInTheDocument();
    await expect(canvas.getByText("선택한 요건이 없다.")).toBeVisible();

    await userEvent.click(canvas.getByRole("button", { name: "요건 검색/선택" }));
    const documentBody = within(canvasElement.ownerDocument.body);
    await userEvent.click(await documentBody.findByRole("button", { name: /REQ-031/ }));
    await expect(canvas.getAllByText("REQ-031")[0]).toBeVisible();
    await expect(canvas.getByText("하네스 UI 요건 추적 보드")).toBeVisible();

    await userEvent.click(canvas.getByRole("button", { name: /선택 해제/ }));
    await expect(canvas.getByText("선택한 요건이 없다.")).toBeVisible();
    await userEvent.click(canvas.getByRole("button", { name: "실행" }));
    await expect(canvas.getByRole("status")).toHaveTextContent("실행 요청: harness:trace 요건 인자 없음");

    await userEvent.click(canvas.getByRole("button", { name: /App Validate/ }));
    await userEvent.click(canvas.getByRole("button", { name: "실행" }));
    await expect(canvas.getByRole("status")).toHaveTextContent("실행 요청: app:validate 요건 인자 없음");
  },
};

export const ReadyWithRequirement: Story = {
  args: { commands, run: { ...readyRun, requirementId: "REQ-031" }, requirements: requirementRows },
  parameters: {
    docs: {
      description: {
        story: "요건을 선택한 준비 상태다. 선택된 명령 카드 내부에서 선택 요건 요약과 선택 해제 동작이 함께 보이는지 확인한다.",
      },
    },
  },
};

export const Running: Story = {
  args: { commands, run: runningRun, requirements: requirementRows },
  parameters: {
    harness: {
      covers: ["명령이 실행 중이면 새 명령을 시작할 수 없고 실행 중임이 표시된다"],
    },
    docs: {
      description: {
        story: "명령 실행 중 상태다. 선택된 명령 카드 내부의 실행 버튼이 비활성화되고 로그가 진행 중인 상태로 표시되는지 확인한다.",
      },
    },
  },
  play: async ({ canvasElement }) => {
    const canvas = within(canvasElement);
    await expect(canvas.getAllByText("실행 중")[0]).toBeVisible();
    await expect(canvas.getByRole("button", { name: "실행 중" })).toBeDisabled();
    await expect(canvas.getByRole("button", { name: /App Validate/ })).toBeDisabled();
    await expect(canvas.getByText(/\[harness:trace\] trace-requirements\.mjs --requirement REQ-031/)).toBeVisible();
  },
};

export const Succeeded: Story = {
  args: { commands, run: succeededRun, requirements: requirementRows },
  parameters: {
    harness: {
      covers: ["실행 중 출력 로그가 실시간으로 표시되고 종료 시 성공 또는 실패가 표시된다"],
    },
    docs: {
      description: {
        story: "명령이 성공으로 종료된 상태다. 종료 코드와 성공 라벨, 로그 마지막 줄이 함께 확인되어야 한다.",
      },
    },
  },
  play: async ({ canvasElement }) => {
    const canvas = within(canvasElement);
    await expect(canvas.getByText("성공")).toBeVisible();
    await expect(canvas.getAllByText("0")[0]).toBeVisible();
    await expect(canvas.getByText(/\[harness:index-requirements\] requirements\.index\.json/)).toBeVisible();
    await expect(canvas.getByText(/\[harness:trace\] trace-requirements\.mjs --requirement REQ-031/)).toBeVisible();
    await expect(canvas.getByText(/gate: pass/)).toBeVisible();
  },
};

export const Failed: Story = {
  args: { commands, run: failedRun, requirements: requirementRows },
  parameters: {
    harness: {
      covers: ["실행 중 출력 로그가 실시간으로 표시되고 종료 시 성공 또는 실패가 표시된다"],
    },
    docs: {
      description: {
        story: "명령이 실패로 종료된 상태다. 실패 라벨과 종료 코드, 로그가 다음 조치 판단에 충분히 표시되는지 확인한다.",
      },
    },
  },
  play: async ({ canvasElement }) => {
    const canvas = within(canvasElement);
    // 종료 시 실패가 표시된다
    await expect(canvas.getByText("실패")).toBeVisible();
    // 종료 코드와 실패 로그가 다음 조치 판단에 표시된다
    await expect(canvas.getByText(/\[harness:trace\] trace-requirements\.mjs --requirement REQ-031/)).toBeVisible();
    await expect(canvas.getByText(/TRACE failed: REQ-031 RED/)).toBeVisible();
  },
};

export const Rejected: Story = {
  args: { commands, run: rejectedRun, requirements: requirementRows },
  parameters: {
    docs: {
      description: {
        story: "허용 목록 밖 명령 요청이 서버에서 거절된 상태다. 거절 사유가 로그와 별도 경고로 표시되는지 확인한다.",
      },
    },
  },
};

export const Loading: Story = {
  args: { commands, run: readyRun, requirements: requirementRows },
  render: () => <LoadingState label="명령 목록을 불러오는 중" />,
  parameters: {
    docs: {
      description: {
        story: "명령 목록 조회 대기 상태다. 빈 명령 목록을 표시하지 않고 로딩 상태를 명확히 보여준다.",
      },
    },
  },
};

export const ErrorStateStory: Story = {
  args: { commands, run: readyRun, requirements: requirementRows },
  render: () => <ErrorState message="명령 실행 상태를 읽지 못했다." />,
  parameters: {
    docs: {
      description: {
        story: "명령 실행 상태 조회 실패 상태다. 오류 메시지와 다시 시도 진입점이 표시되는지 확인한다.",
      },
    },
  },
};
